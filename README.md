...
> **「世界中の『生々しい金儲けの事実と手口』を、出典の有無を隠さず広く吸い上げて蓄積し、読者には『事業のカンニングペーパー』として熱狂させ、自身は『マネーの関所』として莫大な富を抜く。」**

資本主義の裏帳簿（誰が・誰から・どうやって金を抜いたか）の独占的アーカイブと、将来のあらゆる事業（Idea Spark / GOLDMINE / M&A仲介等）で半永久的に使い回すUniversal Data Foundationのフロントエンド兼データレポジトリです。出典は信用状態を決める情報であって、候補を拾う入口の絶対条件ではありません。

## データ収集の共通契約

全体で集める項目と、出典なしでも拾う場合の表示・検証ルールは [データ収集契約](./docs/DATA_COLLECTION_CONTRACT.md) に固定しています。

- 会社名・人物・日付・行動・顧客・価格・集客・技術・競合を集める。
- 売上だけでなく、利益、コスト、利益率、手取りを最優先で分離して集める。
- 出典なしの候補も「未確認」と明示して保存・表示し、後から根拠を追補する。
- 出典がない情報を検証済み・確定実績としては扱わない。
...
[最上部ナビゲーション]
  ...
  ├─ ② 実践アイデア台帳 (IDEAS_VAULT) ─ 手札ゼロから最短で現金を抜く実践アイデア・手口一覧
  └─ ③ 分析台帳 DB (TERMINAL) ────── 22社の実在損益計算書、ウォーターフォール、稼働インフラ
```

### 1. ポータル・特集 (PORTAL)
*   **地雷市場（死体） ⇄ 金脈市場（生者）直接対比レーダー**:
    *   構造的衰退市場（AIアセット直売、国内店舗せどり、無差別LP制作）
    *   資金流入フロンティア（TikTok Shop手元実演、海外OSS代理導入、富裕現場DX送客）
...
*   5大分類（高単価産業中抜き、AI・先端トレンド、初期資本ゼロ、ノーコード運用、ソロ高純利益ストック）で即座に絞り込み可能。
...
5.  **Phase 4: 専用ページ「実践アイデア台帳」の新設**:
    *   ポータル・アイデア台帳・分析DBの三位一体ナビゲーションを確立。
6.  **Phase 5: AI臭さの完全駆逐と「冷徹な機能美（Institutional Minimalism）」の確立**:
    *   絵文字（💡, 💰, ⚡, 🛠️, 🏝️, ⚠️, 🚀, ★等）の完全永久追放。
    *   チンドン屋のような原色バッジを全廃し、白・黒・スレートグレーのモノトーンへ統一。
    *   等幅数字（Tabular Figures）の垂直整列、マッキンゼー式アクションタイトルへの言語昇華。
