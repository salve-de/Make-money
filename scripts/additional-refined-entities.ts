...
export const ADDITIONAL_REFINED_SAMPLE_ENTITIES: FinancialEntity[] = [
  // 1. Photo AI
  {
    ...
    tags: ['ソロプレナー', '他社API中抜き', '粗利85%超', 'AIアバター', '完全放置'],
    pnl: {
    ...
    evidenceCards: [
      {
        ...
        type: 'LOOT_BLUEPRINT',
        title: '【略奪転用】巨大な他社AIモデルを薄皮包装し、写真館のスタジオ代を即座に解消するコード',
        badge: '略奪転用方程式',
        evidenceStatus: 'VERIFIED',
        punchline: '自前で巨大モデルを開発せず、オープンソースモデル（Flux等）をReplicate APIで呼び出し、ユーザーが写真を10枚アップロードするだけのUIに着せる。',
        details: [
          '【Step 1: 痛みの特定】: 「マッチングアプリやLinkedInの写真が必要だが、写真館に行くのが恥ずかしい・高い」という痛みを特定。',
          '【Step 2: 他社APIの薄皮包装】: 自前GPUを持たず、ReplicateのモデルトレーニングAPIに自撮りを流し込んでLoRAを学習させる。',
          '【Step 3: 前金一括課金】: 月額課金ではなく「パック買い切り（$29〜$99）」で前金を総取りし、API原価を大幅に下回る価格で回収。',
        ],
        codeSnippet: '// Replicate APIを用いたLoRAファインチューニング実行スニペット\nimport Replicate from "replicate";\nconst replicate = new Replicate({ auth: process.env.REPLICATE_API_TOKEN });\nexport async function trainModel(userImagesZipUrl: string) {\n  return await replicate.trainings.create("ostris", "flux-dev-lora", "latest", {\n    input: { input_images: userImagesZipUrl, steps: 1000 }\n  });\n}',
        sourceNote: 'Pieter Levelsの公開ビルドログおよびReplicate API連携実績から抽出',
      },
      {
        id: 'ev_photoai_crime',
        type: 'THE_CRIME',
    ...
    operations: {
      ...
      automationLevel: 98,
      primaryChannels: ['創業者Xのビルド実況（オーガニック）', 'TikTokバイラル動画'],
      toolStack: [
        { name: 'Replicate', category: 'AI推論基盤', monthlyCost: 1200000, purpose: 'Flux/SDモデルのLoRA学習および画像推論' },
        { name: 'Stripe', category: '決済代行', monthlyCost: 450000, purpose: '世界中からの即時カード決済' },
        { name: 'Vercel', category: 'ホスティング', monthlyCost: 30000, purpose: 'フロントエンドおよびサーバーレスAPI実行' },
      ],
    },
    strategy: {
  ...
  // 2. Clubhouse (Alpha Exploration) [検死解剖]
  {
    ...
    tags: ['検死解剖', 'VC資金燃焼', '音声単体依存', 'テキスト資産欠落', '急激な過疎化'],
    pnl: {
    ...
    operations: {
      ...
      automationLevel: 30,
      primaryChannels: ['電話帳同期バイラル招待', 'イーロン・マスク等著名人ゲリラ登壇'],
      toolStack: [
        { name: 'Agora.io', category: 'リアルタイム通信', monthlyCost: 80000000, purpose: 'グローバル音声配信SDK' },
        { name: 'AWS', category: 'インフラ', monthlyCost: 20000000, purpose: 'ユーザー管理および通知サーバー' },
      ],
    },
    strategy: {
  ...
  // 3. Quibi [検死解剖]
  {
    ...
    tags: ['検死解剖', '2000億爆死', '素人無料網の見落とし', 'スクショ禁止の自滅', '6ヶ月で清算'],
    pnl: {
      monthlyRevenue: 50000000,
      cogs: 2000000000,
    ...
    operations: {
      ...
      automationLevel: 10,
      primaryChannels: ['スーパーボウルCM巨額出稿', 'ハリウッド大物監督看板'],
      toolStack: [
        { name: '自社動画配信基盤', category: 'インフラ', monthlyCost: 100000000, purpose: 'Turnstyle縦横動画配信' },
        { name: 'DRM著作権保護', category: 'セキュリティ', monthlyCost: 20000000, purpose: '画面録画・スクショ完全遮断' },
      ],
    },
    strategy: {
  ...
  // 4. S'well
  {
    ...
    tags: ['高単価D2C', '粗利60%', '日用品のブランド化', '無借金ブートストラップ', 'リテール棚ハック'],
    pnl: {
    ...
    operations: {
      ...
      automationLevel: 65,
      primaryChannels: ['高級セレクトショップ・百貨店卸', 'スターバックス店頭', 'B2B大口カスタムノベルティ'],
      toolStack: [
        { name: 'Shopify Plus', category: 'EC基盤', monthlyCost: 350000, purpose: 'D2C直販オンラインストア' },
        { name: 'NetSuite', category: 'ERP・在庫管理', monthlyCost: 1500000, purpose: '世界リテール卸の受発注管理' },
      ],
    },
    strategy: {
  ...
  // 5. Chubbies
  {
    ...
    tags: ['D2C', '粗利60%', 'ニッチアパレル', '男友達コミュニティ', 'バイラルマーケ'],
    pnl: {
    ...
    operations: {
      ...
      automationLevel: 70,
      primaryChannels: ['自社ミーム動画（Instagram/TikTok）', 'Klaviyoメールマガジン', '既存顧客リファラル'],
      toolStack: [
        { name: 'Shopify Plus', category: 'ECカート', monthlyCost: 350000, purpose: 'グローバル直販プラットフォーム' },
        { name: 'Klaviyo', category: 'メルマガ自動化', monthlyCost: 1500000, purpose: '顧客セグメント配信・リピート促進' },
      ],
    },
    strategy: {
  ...
  // 6. 2PM
  {
    ...
    tags: ['ソロプレナー', '高単価サブスク', '粗利90%超', 'B2Bリサーチ', '経費で落ちる財布'],
    pnl: {
    ...
    operations: {
      ...
      automationLevel: 85,
      primaryChannels: ['創業者Twitter/LinkedInの知見共有', '既存役員購読者の口コミ'],
      toolStack: [
        { name: 'Ghost', category: 'パブリッシング', monthlyCost: 15000, purpose: '高機能ニュースレター配信' },
        { name: 'Stripe', category: '決済代行', monthlyCost: 240000, purpose: '年額サブスクリプション自動更新' },
      ],
    },
    strategy: {
  ...
  // 7. Baseten
  {
    ...
    tags: ['AIインフラ', 'サーバーレスGPU', '従量課金', 'オープンソース基盤', 'B2B急成長'],
    pnl: {
    ...
    operations: {
      ...
      automationLevel: 80,
      primaryChannels: ['オープンソースTruss経由の開発者流入', 'AIスタートアップコミュニティ'],
      toolStack: [
        { name: 'Kubernetes', category: 'クラスタ制御', monthlyCost: 5000000, purpose: 'GPUリソースの動的プロビジョニング' },
        { name: 'Datadog', category: '監視', monthlyCost: 1500000, purpose: 'モデル推論遅延とエラー監視' },
      ],
    },
    strategy: {
  ...
  // 8. Airgram
  {
    ...
    tags: ['AI議事録', '会議Bot', 'Notion連携', '粗利75%', 'B2B生産性'],
    pnl: {
    ...
    operations: {
      ...
      automationLevel: 90,
      primaryChannels: ['会議同席Botからのバイラル流入', 'Zoom/Teams App Marketplace'],
      toolStack: [
        { name: 'Zoom Bot SDK', category: '連携基盤', monthlyCost: 500000, purpose: '会議室への自動入室と音声取得' },
        { name: 'OpenAI API', category: '推論エンジン', monthlyCost: 3500000, purpose: '文字起こしおよび要約生成' },
      ],
    },
    strategy: {
  ...
  // 9. APUtime
  {
    ...
    tags: ['AI自動計画', '工数削減', '数理最適化', '粗利80%', 'PM向けSaaS'],
    pnl: {
      monthlyRevenue: 15000000,
      cogs: 2500000,
    ...
    operations: {
      ...
      automationLevel: 85,
      primaryChannels: ['欧州B2B展示会・カンファレンス', 'プロジェクト管理連携マーケット'],
      toolStack: [
        { name: 'AWS Lambda / ECS', category: '計算基盤', monthlyCost: 1000000, purpose: '最適化エンジンの高速実行' },
        { name: 'Stripe', category: '決済代行', monthlyCost: 450000, purpose: '企業月額課金' },
      ],
    },
    strategy: {
  ...
  // 10. Capacities
  {
    ...
    tags: ['PKMツール', 'オブジェクト指向', 'Notion代替', '粗利90%', '熱狂的ファンコミュニティ'],
    pnl: {
    ...
    operations: {
      ...
      automationLevel: 85,
      primaryChannels: ['PKM系YouTuberによる自発的レビュー', 'Discord公式コミュニティ', 'Twitter知見共有'],
      toolStack: [
        { name: 'Tauri / Rust', category: 'アプリ基盤', monthlyCost: 0, purpose: '超軽量・爆速デスクトップアプリ' },
        { name: 'Supabase', category: 'バックエンド', monthlyCost: 300000, purpose: 'ユーザー認証およびクラウドデータ同期' },
        { name: 'Stripe', category: '決済代行', monthlyCost: 360000, purpose: '月額/年額Proサブスクリプション' },
      ],
    },
    strategy: {
