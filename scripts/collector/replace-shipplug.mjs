...
const replacementPath = 'data/incoming/batch_shipplug_20260916.json';
const ledgerPath = 'data/CLAIMED_TARGETS.txt';
const oldName = '$3K in 45 Days Selling “Tiny AI Tools” Anyone Could Build';
...
for (const key of ['id', 'name', 'ticker']) { const values = next.map((item) => String(item[key] ?? '').toLowerCase().trim()); if (new Set(values).size !== next.length) throw new Error(`Duplicate ${key} after replacement`); }
