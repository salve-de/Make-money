...
const AGENT_ID = "codex-20260916-ih-new100c";
const BATCH_ID = "batch-indie-hackers-new100c-20260916";
const CLAIMS_FILE = path.join(ROOT, "data/CLAIMED_TARGETS.txt");
const OUT_FILE = path.join(ROOT, "data/incoming/batch_indiehackers_new100c_20260916.json");
const SOURCE_FILE = path.join(ROOT, "data/incoming/batch_indiehackers_new100c_sources_20260916.json");
const RECEIPT_FILE = path.join(ROOT, "data/incoming/batch_indiehackers_new100c_official_checks_20260916.json");
...
async function fetchPool() {
  ...
  for (const query of TERMS) {
    const params = new URLSearchParams({ hitsPerPage: "1000", page: "0", query, numericFilters: "revenue>=2000,revenue<=100000" });
...
function eligible(item) {
  ...
  return name.length >= 3 && tagline.length >= 12 && description.length >= 20 && /^https?:\/\//i.test(url)
    && domain(url) && year >= 2018 && year <= 2026 && !social.test(url)
...
function buildEntity(item, audit) {
  ...
  return {
    ...
    strategy: {
      blindspot: `「${tagline}」という狭い課題への集中。競合の見落とし・大手の自縛は未確認。`, moatType: "UNKNOWN", moatDescription: "継続率・独自データ・供給制約・ブランド優位は未確認。", incumbentDilemma: "大手のカニバリゼーション障壁は未確認。", secretInsight: `公開記録から確定できるのは、${tagline}という提供表現と売上表示だけ。`, initialTraction: [`掲載開始値: ${item.startDateStr || "UNKNOWN"}`, `公開レコードに掲載。${revenueLabel}`, "初動獲得経路・顧客数・継続率は未確認。"], actionPlaybook: [`「${tagline}」の当事者に対象を絞り、支払理由を一次情報で確認する。`, `報告売上${revenueLabel}を利益と分離し、原価・経費・税・継続率を取得する。`, `公式URLの提供条件と掲載説明を同一IDで保存する。`], coldOutreachTemplate: `「${tagline}」に関する作業で、最も時間または損失が出る箇所を15分だけ教えてください。公開説明だけで利益を断定せず、実測値と照合します。`,
