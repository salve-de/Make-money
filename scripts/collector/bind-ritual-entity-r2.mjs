...
const r2Path = 'data/incoming/raw_snapshots_ritual_retreats_20260916.r2-result.json';
...
if (!r2?.payloadReadback || !r2?.manifestReadback) throw new Error('Ritual Retreats raw R2 readback is incomplete');
const entity = batch.find((item) => item.id === replacement.id);
if (!entity) throw new Error(`Ritual Retreats entity not found: ${replacement.id}`);
