...
const AGENT_ID = "codex-20260916-ih-new100n";
const BATCH_ID = "batch-indie-hackers-new100n-20260916";
const CLAIMS_FILE = path.join(ROOT, "data/CLAIMED_TARGETS.txt");
const OUT_FILE = path.join(ROOT, "data/incoming/batch_indiehackers_new100n_20260916.json");
const SOURCE_FILE = path.join(ROOT, "data/incoming/batch_indiehackers_new100n_sources_20260916.json");
const RECEIPT_FILE = path.join(ROOT, "data/incoming/batch_indiehackers_new100n_official_checks_20260916.json");
...
async function fetchPool() {
  ...
  for (const query of TERMS) {
    const params = new URLSearchParams({ hitsPerPage: "1000", page: "0", query, numericFilters: "revenue>=100,revenue<=499" });
...
function eligible(item) {
  ...
  return name.length >= 3 && tagline.length >= 12 && description.length >= 20 && /^https?:\/\//i.test(url)
    && domain(url) && year >= 2024 && year <= 2026 && tags.some((tag) => ["founders-solo", "employees-0", "employees-under-10", "employees-10-plus"].includes(tag)) && !social.test(url)
...
function selectClaimed(pool) {
  ...
  const eligiblePool = pool.filter(eligible);
  const selected = wanted.map((target) => eligiblePool.find((item) => normalize(item.name) === normalize(target.name) && domain(item.websiteUrl) === target.domain)).filter(Boolean);
  if (selected.length !== wanted.length) {
    const missing = wanted.filter((target) => !eligiblePool.some((item) => normalize(item.name) === normalize(target.name) && domain(item.websiteUrl) === target.domain));
