...
const oldName = 'He Turned A Dead Travel Blog Into A $10K/Month Business';
...
for (const key of ['id', 'name', 'ticker']) {
  const normalized = next.map((item) => String(item[key] ?? '').toLowerCase().trim());
  if (new Set(normalized).size !== next.length) throw new Error(`Duplicate ${key} after replacement`);
}
