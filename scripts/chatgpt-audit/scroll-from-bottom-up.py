...
# We are at bottom. Let us scroll UP progressively and capture each screen
for i in range(1, 12):
    scroll(15, count=5) # Scroll UP
    img_path = f"{out_dir}/chatgpt_from_bottom_up_{i}.png"
    subprocess.run(['screencapture', '-x', '-R700,33,740,875', img_path], check=True)
    print(f"Captured up_{i}")
