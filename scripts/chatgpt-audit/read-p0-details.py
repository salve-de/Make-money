...
# We are currently around r9_page_1. Let's scroll down slightly 4 times and capture
for i in range(1, 5):
    for _ in range(6):
        ev = cg.CGEventCreateScrollWheelEvent(None, 1, 1, -8) # scroll down
        cg.CGEventPost(0, ev)
        cg.CFRelease(ev)
        time.sleep(0.02)
    time.sleep(0.4)
    subprocess.run(["screencapture", "-x", "-R700,33,740,875", f"{out_dir}/chatgpt_detail_down_{i}.png"], check=True)
    print(f"Captured detail_down_{i}")
