...
```text
app/ （ルーティング・グローバル設定・配線のみ。JSXは50行以内）
  ↓
features/ （ビジネス価値を提供する独立機能スライス）
  ↓
shared/ （ドメイン知識ゼロの共通部品・ユーティリティ・基盤UI）
```
- **絶対ルール（単方向依存）**:
...
- **行数の目安（警報装置）**:
  - **300行超**: 分割検討（注意ゾーン）。
  - **500行超**: 強い分割候補（原則CI警告/エラー。設定・スキーマ・モックデータ・自動生成コード等は例外）。
- **分割すべき5大兆候（Smells）**: 以下のいずれかが発生した瞬間に物理分割せよ：
...
- 外部入力は入口（Boundary）において Zod / JSON Schema で100%検証してから内部型へ流し込め。
