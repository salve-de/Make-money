...
## 9. 【画面表示逆算型データ収集憲法】表示要件を満たさないカスデータの収集・公開完全禁止（Display-First Collection & Pipeline Contract）

「名前やURLだけのカスデータを集めてゴミ屋敷を作る」「画面に出せない情報を集めて満足する」「未精錬データを画面に流し込んで読者の信頼を破壊する」という知能退行を永久に撲滅するための絶対規範。
すべてのAIおよびエージェントは、**「画面（UI）にキーエンス品質・Stripe基準で具体的に表示できるか」から完全に逆算してデータを収集・精錬せよ**。

### 1. 収集の合否判定：画面必須4大ピース（これらが揃わない収集は完全失格）
収集対象の企業・事例は、以下の4大ピースが揃うまで「収集完了」とみなしてはならない。1つでも欠落している場合は、自律的に2次・3次の深掘り外部検索を行い、ピースを埋めきれ：
1. **【一覧1行リード（60〜90字）】**:
   - 顧客の痛みの財布 ＋ 具体的な仕掛け・ズル ＋ 商売上の帰結（数字）が凝縮された日本語。「〇〇向けのSaaS」といった説明書や記事タイトルは厳禁。
2. **【事業DNA（3行構造）】**:
   - ①何屋か（身も蓋もない一行）／②誰の財布（人質にした感情・保身・恐怖）／③切除する苦痛（顧客が理性を失って即決する理由）の具体的ファクト。
3. **【泥臭い初動突破ログ（最初の100人を仕留めた実録）】**:
   - 「価値ある発信で伸ばした」という欺瞞を剥ぎ、創業者がRedditで自演したログ、手作業DM、規約の隙間ハック等の客観事実。
4. **【客観的損益P&L（公開確定値 または 明示的推計値）】**:
   - 単価・顧客規模・Stripe決済手数料（2.9%＋$0.30）・推論API原価・インフラ費を因数分解した数字。推計の場合は計算式と根拠（`origin_type: 'estimated'`）を必ず添付。

### 2. 「保存数」と「公開事例数」の厳格な分離
- **未精錬データの画面露出厳禁**: R2に存在する2,000件超の候補オブジェクトを、未精錬・中身スカスカのままUI一覧へ露出させる行為は、読者が離脱する自殺行為である。
- **公開基準**: 画面（UI一覧・詳細）に並ぶのは、上記4大ピースを完備し、Stripe基準の高密度・具体性を満たした「完全体事例（Dossier）」のみとする。
- **段階的昇格プロトコル**: R2の未精錬候補から、有益な事実・初動・数字が揃ったものから順次精錬（Gold化）し、公開事例数（123件 ➔ 200件 ➔ 500件）として1件ずつ着実に積み増せ。

### 3. 2重保存パイプライン（共有原油 ＋ 即時表示Gold）
収集したデータは、以下の2つの出口へ同時に流し込め：
1. **【全プロジェクト共有資産（原油）】**:
   - `universal-foundation` 規格（46項目、Entity, Claim, MoneySignal, Event, Observation等）に準拠し、Cloudflare R2（`foundation-lake`）へ Create-Only で永久保存。将来の別プロジェクト（投資・自動化・メディア）でも一生使えるデータグラフとする。
2. **【Make-Money即時表示（Gold）】**:
   - 画面描画用インデックス（`data/entities-index.json` / `mockLedgerData.ts`）へ、サバンナOS直撃の日本語DNA、動的証拠カード（略奪転用方程式）を配備して即座に描画可能とせよ。
