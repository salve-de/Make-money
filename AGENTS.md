> **収集基盤北極星 / 完全判定:** 「現在必要な項目が埋まった」「rightsを後付けできる」だけでは完全ではない。将来未知の任意項目を、既存canonicalを壊さず、既存Evidenceで足りれば再収集せず、versioned Annotation/Enrichmentとして過去全件へbackfill・replay・index・view rebuildできることが北極星。汎用extension layerが実装・テストされるまでは収集基盤全体を完全と呼ばない。詳細は [docs/COLLECTION_TO_UI_MASTER_HANDOFF.md](docs/COLLECTION_TO_UI_MASTER_HANDOFF.md) Section 0 と Universal Foundation lifecycle master Section 0 を正本とする。
...
> 
> **【収集エージェントの鉄則（限定の完全排除 ＆ 周辺・隣接生態系の全量吸い上げ）】**:
> 1. **あらゆる業種・規模・形態が対象**: SaaS、製造、物販、店舗、メディア、下請け、大企業、個人、表、裏など、いかなる限定も一切禁止する。資本主義で現実に金が動いている（または尽きて死んだ）事実は全て等しく収集対象である。
> 2. **企業単体に閉じない「周辺・隣接・裏の生態系5大重力場」の全量吸い上げ**:
>    - 企業単体のスペック調査で終わるな。その周囲で金が動いている重力場全体（①サプライヤー・下請け・API等の黒子の原価構造、②法規制・許認可・プラットフォーム規約の隙間・グレーゾーン、③客による異常な裏転用・二次流通・非公式コミュニティ、④競合の誰が最も血を流しているかの死線、⑤出資者・提携・利権分配の人間関係）を枠組みなしで柔軟に全て吸い上げよ。
> 3. **収集項目と足切り条件の完全分離**: 人数・粗利・原価・URL・創業者・ツール構成・周辺生態系は**当然すべて徹底調査して記録する**。しかし、**いかなる項目の有無や数値も【収集の足切り条件（フィルター）】には絶対にしない**。
...
- **マルチチャット並列自走規律（排他予約 ＆ 隔離保存）**:
  - ① **調査直前の予約ロック（CLAIM_LOCK義務）**: 調査直前に必ず `data/CLAIMED_TARGETS.txt` を検索。掲載済みなら他チャットが調査中または登録済みのため即スキップ。未掲載なら末尾に社名を1行追記してから調査着手。
  - ② **中央台帳の直接編集禁止（`data/incoming/` 保存）**: `data/entities-index.json` を外部チャットから直接編集するな。`data/incoming/batch_[識別名]_[タイムスタンプ].json` に完全体JSON配列（`FinancialEntity[]`）として保存せよ。
  - ③ **手堅い商売・短期高レバレッジ型の全量受入 ＆ 避けるべきゴミの境界**: 派手なズルがなくても現実に利益が出ている手堅い商売（リピート清掃、町工場、個人SaaS等）や、AI等の最新ツールを活用して短時間の開発で一気に立ち上げ利益を抜いた現代的高レバレッジ事例は100%全量拾う。落とすべきは「売上$0の実験場、月数千円のお小遣い稼ぎ、趣味ブログ、学生の工作など事業実体のないノイズ」のみ。個人なら飯が食えている規模（月商数十万〜100万円以上）、企業なら事業として自立している実体を柔軟に拾い上げよ。
- **オーナー（ユーザー）が真に求めている5大最高意志（設計思想）**:
...
- **【収集ルールの絶対禁止条項：SaaSバイアス・手抜きハードコードの完全根絶】**:
  ...
  - ④ **柔軟収集 ＆ 全項目必須の完全撤廃（ゼロ足切り・欠損誠実受入原則）**:
    - **絶対に全ての項目が必要なわけではない**: 初動のズル1本、またはP&L実額、または死因ログなど、資本主義で金が動いた（または死んだ）生々しい事実が1つでもあれば、それだけで即座に拾い上げよ。
...
- **周辺・深層（3大死角 ＆ 市場の歪み ＆ 周辺・隣接5大重力場）**:
  - 削除された黒歴史・初期魚拓、フォーラムの生々しい解約怨嗟（サクラレビューの排除）、真の裏集客配管（SEOやアフィリエイト等のドル箱）。
  - ① サプライヤー・下請け・黒子の原価構造（誰が作っているか、仕入れ値、利益率）。
  - ② 法規制・許認可・プラットフォーム規約の隙間（グレーゾーン、法改正の影響）。
  - ③ 顧客の異常な生態系（公式外の裏転用、二次流通、非公式コミュニティ、信者の布教動機）。
  - ④ 競合の死線（この企業が伸びて業界の誰が最も血を流しているかの構造的被害者）。
  - ⑤ 資本・ステークホルダーの人間関係（出資者、提携、創業者の前職人脈、仲間割れの配分）。
- **【最重要・AI自律判断】枠外の全観測（『そういう系』は限定せず全て吸い上げよ）**:
  - 上記リストは思考の呼び水（チェックボックス）であり、収集の上限（天井）では断じてない。「これ以外は調べない」「型にないから書かない」はAIの完全失格である。
  - スキーマの型（10大属性等）に収まらない周辺情報・生態系・現場の裏ワザ・業界の悪習などは、**AI自身が有益と判断したものは一切限定せず全て吸い上げ、`observations`（上限なしの客観観測ログ配列）および `evidenceCards`（動的カード）へ1文字も捨てずに全量記録せよ**。
  - Web魚拓・原本の生バイト列（HTML/PDF）はRaw CAS（`foundation-raw`）に完全不変で永久保存されるため、後からいかなる周辺情報も掘り起こして新機能・新切り口へ変換できる。
- **保存 ＆ 画面即時反映（完全並行サイクル）**:
