...
- **【収集ルールの絶対禁止条項：SaaSバイアス・手抜きハードコードの完全根絶】**:
  ...
  - ④ **コンテンツ完全密度基準（Fail-Closed Gate）の絶対義務**:
    - `observations` は最低4件以上の一次事実ログ（確定財務、初動集客、業界死角、定着仕組み、運用インフラ等）を完全収録せよ（「メモ1行」等の薄いデータは機械的に即時 reject）。
    - エビデンスカードは3枚すべてにおいて `details` に最低3箇条以上の構造的詳細を記録せよ（空配列や1行は不可）。
    - 初動集客手順（`executionChecklist`）は最低3ステップ以上を具体的に記録せよ。
    - 事業概要（`essence`）の各項目は最低40文字以上の重厚な解説を記録せよ。
    - ユーザーが見た時に意味不明となる社内造語・スラング（「死角」「略奪」等を単体で放置するなど）は完全排除し、客観的・学術的・平易な日本語で記録せよ。
    - これらの条件は `scripts/architecture/check-ingest-quality.mjs` および `real-ingest-pipeline.ts` で物理的に強制され、1件でも違反があればコミット・ビルド・インジェストが不可能となる。
