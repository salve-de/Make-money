...
1. **手作業ゼロ・データ表示率100%の保証**:
   - UIは固定Coreを通常のCompositionで描画し、型がある動的特異点だけをTyped Registryでリッチに可視化する。型に収まらない自由な観測はLayer 3のストリームへ送る。固定UIまで一つの汎用Registryへ押し込まない。
...
## 9. 【画面表示逆算 ＆ 全量底引き網収集憲法】枠による限定完全排除 ＆ 3層UI全量放出規律（Full-Spectrum Vacuum & Display-Driven Ingest Contract）
...
取った膨大なデータを捨てることなく、画面側で以下のように100%活用・表示せよ：
- **Layer 1（Core）**: 一覧1行リード、3行DNA、確定P&Lテーブル（0.1秒で直感理解する最前面）。
- **Layer 2（動的特異点ブロック）**: その企業の最もエグい手口（略奪転用方程式、大手の自爆、寄生配管、死因検死など）。
- **Layer 3（万能救済ストリーム #13）**: 枠に収まらない観測事実、解約怨嗟、時系列、周辺文脈をすべてタイムライン・カードとして**全量ストリーム表示**（取った情報の一切の非表示・死蔵を禁止）。

### 3. 「保存数」と「公開事例数」の規律
- R2事実レイク（`foundation-lake`）には上記すべてを共有原油として Create-Only で永久保存する。
- 画面（UI一覧）に並べるのは、中身スカスカの未精錬候補ではなく、上記「全部」が高純度・高密度に揃った「完全体事例（Dossier）」のみとし、精錬が完了したものから順次公開件数を積み増せ。
