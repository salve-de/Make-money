...
## 7. 【最高峰フロントエンド設計・長期保守憲法】巨大モノリス根絶 ＆ 責任所在完全局所化規律（Anti-Monolith & High-Maintainability Constitution）

「1ファイル1,000〜2,000行のGod Componentを放置し、閉じタグ1つのズレで全体が汚染される」「どこを直せばいいか分からない」「AIが差分編集で構文を破壊する」という恥辱的構造欠陥を永久に撲滅するための絶対規範。
すべてのAIおよび開発者は、Google、Meta、Stripe、Netflix、Airbnb等の世界標準設計およびソフトウェア工学（認知心理学・結合度理論）に基づき、以下を外科医の如く遵守せよ。

### 1. コンポーネント三次元物理上限値（3D Component Physical Limits）
コードを書くすべてのエージェントは、以下の数値上限を**1文字・1行たりとも超えてはならない**。超過したコードは「欠陥品・即時解体対象」とみなす：
1. **【ファイル行数上限】最大300行（推奨150〜200行）**:
   - 1ファイルが300行を超えた瞬間、そのコンポーネントは責務過多（God Component）である。即座にサブコンポーネントまたはカスタムフックへ解体せよ。
2. **【コンポーネント関数上限】最大80行**:
   - 1つのコンポーネント関数の本体（ロジック＋JSX）は80行以内に収めよ。長大なレンダリングブロックは関心事ごとに小コンポーネントとして外へ切り出せ。
3. **【JSXネスト深度上限】最大4階層**:
   - `<div>` やコンポーネントが4重を超えてネストする構造は即座に別名コンポーネントとして抽出し、平坦（Flatter JSX）に保て。多重ネストは閉じタグズレ（Syntax Drift）の温床である。
4. **【循環的複雑度上限】Cyclomatic Complexity 最大10**:
   - 単一関数内での三項演算子のネスト、多重 `if`、`&&` の乱用を厳禁とする。分岐は動的レジストリ（Registry Pattern）またはポリモーフィズムへ逃がせ。
5. **【1ファイル1コンポーネント厳守】（`no-multi-comp`）**:
   - 1つのファイル内に複数のReactコンポーネントを同居させるな。ヘルパーコンポーネントであっても独立ファイルへ切り出せ。

### 2. 単一責任原則（SRP）の物理的強制（Four-Responsibility Isolation）
コンポーネント内に以下の4大責務のうち**「2つ以上」が同居している場合、外科手術の如く物理的に切り離せ**：
1. **データ取得・通信・キャッシュ（Network / Cache）**: `fetch`, React Query, SWR ➔ **カスタムフック / Server Component**へ隔離
2. **状態遷移・計算ロジック（State / Business Logic）**: バリデーション、フィルタリング、ステップ計算 ➔ **カスタムフック（`use[Feature].ts`）**へ隔離
3. **イベント調停・画面配線（Event Orchestration）**: 親子間通知、URL同期、モーダル開閉 ➔ **Container / Page（50行以内）**へ隔離
4. **純粋な視覚描画（Pure Rendering）**: HTMLタグ、Tailwindクラス、レイアウト ➔ **Presenter / View Component**へ隔離

### 3. 動的レジストリパターン（Section / Evidence Registry Pattern）の義務化
カード、セクション、タブなどのバリエーションを親コンポーネント内で `switch (type)` や `&&` でベタ書きすることを厳禁とする（Netflix BDUI / Stripe 採用方式）：
- **親の責務**: 親コンポーネント（Inspector等）は、レジストリからコンポーネントを取得してマップするだけの**「配管（50行以内）」**に徹せよ。
- **子の責務**: 各セクション（財務計器盤、ツールスタック、万能ストリーム等）は**独立したファイル（150行以内）**として実装し、`registry/<domain>Registry.ts` にキーとペアで登録せよ。
- **開放閉鎖原則（OCP）**: 新しいセクションを追加・改修する際、親コンポーネントのコードには1文字も触れてはならない。新ファイルを作成してレジストリに1行追加するだけで完結させよ。

### 4. 完全Colocation原則（Kent C. Dodds 原則）
「一緒に変化するものは、物理的に最も近くに配置せよ」。プロジェクト全体にバラバラに散らすことを禁じ、コンポーネント単位のディレクトリに集約せよ：
```text
components/[FeatureName]/
├── [FeatureName].tsx          # メインPresenter（150行以内）
├── [FeatureName].types.ts    # Propsおよび専用型定義（インライン型の禁止）
├── [FeatureName].hooks.ts    # 状態操作・カスタムフック
└── index.ts                   # 外部公開用Public API
```
- **効果**: 修正時に開くべきファイルが1箇所に集約され、不要になった際はディレクトリごと削除するだけで完全にデッドコードを消滅させられる。

### 5. AI差分編集のための機械的防御（Syntax Drift Prevention）
AIエージェントが大規模ファイルを編集する際の構文破壊を構造的に防ぐため、以下を義務付ける：
1. **事前構文スキャン義務**: ファイル編集の前後で、必ずTypeScript ASTパーサー等を用いてタグ開閉不一致（Mismatched Tags）を機械検査せよ。
2. **一括全量置換の禁止**: 1,000行超の巨大ファイルを1回のプロンプトや置換で書き換えるな。コンポーネント単位に分割してから、局所的に安全な変更を行え。
