    ...
    <section id="section-playbook" className="scroll-mt-4">
      ...
      <div className={`rounded-md border bg-[#10131C] overflow-hidden ${
        ...
        <div className="divide-y divide-white/[0.08]">
              ...
              <div className="flex items-center justify-between flex-wrap gap-2">
                ...
                {hasAcquisition && entity.acquisition && (
                  <div className="flex items-center gap-2 font-mono text-[11px]">
                    ...
                    <span className={`font-semibold ${isHazardMode ? 'text-red-400' : 'text-[#F4F5F7]'}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="p-3 rounded bg-[#151926] border border-white/[0.08] space-y-1">
                  <div className="font-mono text-[11px] font-semibold text-zinc-300 flex items-center gap-1.5">
                    <span className={isHazardMode ? 'text-red-400' : 'text-zinc-500'}>{'//'}</span>
                    <span className="text-[#F4F5F7]">{isHazardMode ? '初期の過熱と数字の錯覚' : '創業初期の集客手口'}</span>
                  </div>
                ...
                <div className="p-3 rounded bg-[#151926] border border-white/[0.08] space-y-1">
                  <div className="font-mono text-[11px] font-semibold text-zinc-300 flex items-center gap-1.5">
                    <span className={isHazardMode ? 'text-red-400' : 'text-zinc-500'}>{'//'}</span>
                    <span className="text-[#F4F5F7]">{isHazardMode ? '規約変更・依存の代償' : 'プラットフォーム規約の隙間'}</span>
                  </div>
                  <p className="text-xs text-zinc-300 leading-relaxed font-sans">
                ...
                <div className="p-3 rounded bg-[#151926] border border-white/[0.08] space-y-1">
                  <div className="font-mono text-[11px] font-semibold text-zinc-300 flex items-center gap-1.5">
                    <span className={isHazardMode ? 'text-red-400' : 'text-zinc-500'}>{'//'}</span>
                    <span className="text-[#F4F5F7]">事業転換（ピボット）と死線</span>
                  </div>
                  <p className="text-xs text-zinc-300 leading-relaxed font-sans">
                ...
                <div className="p-3 rounded bg-[#151926] border border-white/[0.08] space-y-1">
                  <div className="font-mono text-[11px] font-semibold text-zinc-300 flex items-center gap-1.5">
                    <span className={isHazardMode ? 'text-red-400' : 'text-zinc-500'}>{'//'}</span>
                    <span className="text-[#F4F5F7]">{isHazardMode ? '原価高騰と固定費出血' : '実際に使っていたツールと原価'}</span>
                  </div>
                  <p className="text-xs text-zinc-300 leading-relaxed font-sans">
            ...
            <div className="p-4 space-y-3">
              ...
              <div className="space-y-1.5">
                {entity.strategy.actionPlaybook.map((step, idx) => (
                  <div key={idx} className="flex items-start gap-3 text-xs text-zinc-200 p-2.5 rounded bg-[#151926]/40 border border-white/[0.04] hover:bg-white/[0.02] transition-colors">
                    <span className={`font-mono text-[11px] font-bold shrink-0 mt-0.5 ${
                      isHazardMode ? 'text-red-400' : 'text-zinc-400'
                    }`}>
                      STEP {String(idx + 1).padStart(2, '0')}
                    </span>
                    <span className="leading-relaxed font-sans flex-1 text-zinc-200">{step}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {hasProAnalysis && (
            <div className="p-4 space-y-3 bg-[#131724]/40">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className={`font-mono text-[10px] font-bold px-1.5 py-0.5 rounded ${
                    isHazardMode ? 'text-red-300 bg-red-950/60 border border-red-500/40' : 'text-zinc-200 bg-white/[0.08] border border-white/[0.12]'
                  }`}>
                    PRO INTELLIGENCE
                  </span>
                  <span className="font-mono text-[11px] font-semibold tracking-wider text-zinc-300 uppercase">
              ...
              {isPro && entity.meta ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
                  {/* #01 大手のジレンマ */}
                  <div className="p-3 rounded bg-[#151926] border border-white/[0.08] space-y-1.5">
                    ...
                    <div className="text-[11px] text-zinc-300 space-y-1 font-sans">
                      <div><span className="text-zinc-400 font-mono">障壁:</span> {entity.meta.incumbentDilemma.cannibalizationBarrier}</div>
                      <div><span className="text-zinc-400 font-mono">規模不一致:</span> {entity.meta.incumbentDilemma.scaleMismatchReason}</div>
                      <div><span className="text-zinc-400 font-mono">速度優位:</span> {entity.meta.incumbentDilemma.decisionSpeedAdvantage}</div>
                    </div>
                  </div>
                  ...
                  <div className="p-3 rounded bg-[#151926] border border-white/[0.08] space-y-1.5">
                    ...
                    <div className="text-[11px] text-zinc-300 space-y-1 font-sans">
                      <div><span className="text-zinc-400 font-mono">比較基準:</span> {entity.meta.pricingPower.anchorComparison}</div>
                      <div><span className="text-zinc-400 font-mono">損失回避:</span> {entity.meta.pricingPower.lossAversionTrigger}</div>
                      <div><span className="text-zinc-400 font-mono">財布区分:</span> {entity.meta.pricingPower.budgetCategory}</div>
                    </div>
                  </div>
                  ...
                  <div className="p-3 rounded bg-[#151926] border border-white/[0.08] space-y-1.5">
                    ...
                    <div className="text-[11px] text-zinc-300 space-y-1 font-sans">
                      <div><span className="text-zinc-400 font-mono">人質データ:</span> {entity.meta.lockInMechanism.dataHostage}</div>
                      <div><span className="text-zinc-400 font-mono">業務統合:</span> {entity.meta.lockInMechanism.workflowIntegration}</div>
                      <div><span className="text-zinc-400 font-mono">乗換摩擦:</span> {entity.meta.lockInMechanism.switchingFriction}</div>
                    </div>
                  </div>
                  ...
                  <div className="p-3 rounded bg-[#151926] border border-white/[0.08] space-y-1.5">
                    ...
                    <div className="text-[11px] text-zinc-300 space-y-1 font-sans">
                      <div><span className="text-zinc-400 font-mono">入金サイクル:</span> {entity.meta.capitalEfficiency.cashConversionCycle}</div>
                      <div><span className="text-zinc-400 font-mono">限界粗利:</span> {entity.meta.capitalEfficiency.incrementalMargin}</div>
                      <div><span className="text-zinc-400 font-mono">運転資本:</span> {entity.meta.capitalEfficiency.workingCapitalStrategy}</div>
                    </div>
                  </div>
                </div>
              ) : !isPro ? (
