...
interface LaptopTableViewProps {
...
export const LaptopTableView: React.FC<LaptopTableViewProps> = ({
  companies,
  onSelectCompany,
  selectedCompanyId,
  onSortByRevenue,
  ...
  return (
    <table className="hidden md:table xl:hidden table-fixed w-full text-left border-collapse font-sans text-xs select-none">
      {/* 厳格な固定幅ヘッダー */}
      <thead className="bg-[#0F131C] text-[10px] font-mono text-zinc-400 uppercase tracking-wider sticky top-0 z-10 border-b border-white/[0.08]">
        <tr>
          <th className="w-10 py-2.5 px-2.5 text-center">#</th>
          <th className="w-[34%] py-2.5 px-3">企業名 / モデル</th>
          <th 
            onClick={onSortByRevenue}
            className="w-[20%] py-2.5 px-3 text-right cursor-pointer hover:text-white"
          >
          ...
          <th className="w-[22%] py-2.5 px-3 text-right">実効手残り純利</th>
          <th 
            onClick={onSortByMargin}
            className="w-[14%] py-2.5 px-3 text-right cursor-pointer hover:text-white"
          >
            <div className="flex items-center justify-end gap-1">
              <span>利益率</span>
              <ArrowUpDown size={10} className="text-zinc-500" />
          ...
          <th className="w-[10%] py-2.5 px-2.5 text-center">体制 / 解剖</th>
        </tr>
      </thead>
      ...
      <tbody className="divide-y divide-white/[0.05] bg-[#080B10] font-sans">
        {companies.map((company, index) => {
          ...
          return (
            ...
            >
              {/* # */}
              <td className="py-2.5 px-2.5 text-center font-mono text-[11px] text-zinc-400 font-bold">
                {index + 1}
              </td>
              ...
              <td className="py-2.5 px-3">
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-[10px] text-zinc-400 truncate mt-0.5 max-w-[220px]">
                      {company.tagline}
                    </div>
                  </div>
                </div>
              ...
              <td className="py-2.5 px-3 text-right whitespace-nowrap">
                <span className={`text-[11px] font-bold font-mono tabular-nums px-2 py-0.5 rounded inline-block max-w-[160px] truncate ${
                  profit < 0
              ...
              <td className="py-2.5 px-2.5 text-center">
                <div className="flex items-center justify-center gap-1.5">
                  <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-white/[0.04] text-zinc-400 border border-white/[0.06]">
                    {company.teamSize === 1 ? '1人' : company.scaleTier === 'MEGA_CORP' ? '独占' : `${company.teamSize}名`}
                  </span>
