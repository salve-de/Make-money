...
function MobileCompanyRow({ company, selected, saved, compared, onSelect, onSave, onCompare }: { company: CompanyRecord; selected: boolean; saved: boolean; compared: boolean; onSelect: () => void; onSave: () => void; onCompare: () => void }) {
  const financial = latest(company);
  return <article className={`mm-mobile-result ${selected ? 'is-selected' : ''}`} onClick={onSelect}>
    <div className="mm-mobile-result-head"><div className="mm-company-cell"><span className="mm-company-logo">{company.japaneseName.slice(0, 1)}</span><span><strong>{company.japaneseName}</strong><small>{company.ticker} · {company.scaleTier.replace('_', ' ')}</small></span></div><span className={`mm-evidence evidence-${company.verifiedStatus.toLowerCase()}`}>{evidenceLabel(company.verifiedStatus)}</span></div>
    <p className="mm-mobile-model">{company.businessEssence?.monetizationWay || company.businessModel}</p>
    <div className="mm-mobile-metrics"><div><span>売上高</span><strong>{money(financial.revenueJpy)}</strong><small>{financial.period}</small></div><div><span>営業利益率</span><strong>{financial.operatingMarginPercent.toFixed(1)}%</strong></div><div><span>成長率</span><strong className={growth(company) >= 0 ? 'mm-up' : 'mm-down'}>{growth(company) >= 0 ? '+' : ''}{growth(company).toFixed(1)}%</strong></div></div>
    <div className="mm-mobile-result-actions" onClick={(event) => event.stopPropagation()}><button className={compared ? 'is-on' : ''} onClick={onCompare}><BarChart3 size={14} />{compared ? '比較中' : '比較'}</button><button className={saved ? 'is-on' : ''} onClick={onSave}><Bookmark size={14} fill={saved ? 'currentColor' : 'none'} />{saved ? '保存済み' : '保存'}</button><button className="mm-mobile-open" onClick={onSelect}>詳細 <ChevronRight size={14} /></button></div>
  </article>;
}
