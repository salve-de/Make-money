        ...
        <div className="relative rounded-lg border border-white/[0.08] bg-[#07080B] p-2 shadow-2xl overflow-hidden text-left">
          <div className="flex items-center justify-between px-3 py-2 border-b border-white/[0.06] text-[10px] font-mono text-zinc-500">
            <div className="flex items-center gap-2">
              ...
              <div className="w-2 h-2 rounded-full bg-zinc-700" />
              <div className="w-2 h-2 rounded-full bg-zinc-700" />
              <span className="ml-2 text-zinc-400 font-medium">保存済み台帳のプレビュー（時点・根拠は詳細へ）</span>
            </div>
          ...
          <div className="p-4 bg-[#060709] font-mono text-xs overflow-x-auto">
            ...
            <div className="divide-y divide-white/[0.04]">
              {entities.filter((entity) => ['ent_photoai', 'ent_keyence', 'ent_stripe'].includes(entity.id)).map((entity) => {
                const snapshot = financialSnapshot(entity);
                return (
                  <Link key={entity.id} href={`/?entity=${entity.id}`} className="grid grid-cols-6 gap-4 py-2.5 items-center min-w-[640px]">
                    <span className="font-semibold text-white">{entity.name}</span>
                    <span className="text-right text-zinc-300">{snapshot.revenue}/月</span>
                    <span className="text-right text-emerald-400 font-medium">{snapshot.margin}</span>
                    <span className="text-zinc-500">{entity.scale}</span>
                    <span className="col-span-2 text-zinc-400 truncate text-[11px]">{entity.tagline}</span>
                  </Link>
                );
              })}
            </div>
