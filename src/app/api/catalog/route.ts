...
export async function GET(request: Request) {
  ...
  if (query.length > 200 || !Number.isSafeInteger(offset) || offset < 0) return NextResponse.json({ error: 'Invalid catalog query' }, { status: 400 });
  ...
  try {
    ...
    const data = filtered.slice(offset, offset + 100).map(publicSummaryEntity);
