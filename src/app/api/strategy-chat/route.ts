...
function generateFallbackChatResponse(
  ...
  if (q.includes('集客') || q.includes('顧客') || q.includes('マーケ') || q.includes('トラクション')) {
    ...
    reply = `【結論: データを見ろ。お前の「固定費ゼロ」は大手が逆立ちしても勝てない最大の構造的優位だ】\n\n` +
      `適当な励ましではない。冷徹なP&Lと構造ファクトから、お前が確実に勝てる3大根拠を提示する:\n\n` +
      `1. 大手の自爆（カニバリズム）: ${entity.name}の事例を見よ。大手が年商数億円の既存基盤を守るために手を出せない「利益率80%超の超特化領域」こそ、お前が無傷で月利100万円を抜くための完全な独壇場だ。\n` +
      `2. 痛みの財布に針を刺せ: ${entity.targetPainWallet || '顧客の損失恐怖と怠惰'}を直撃しろ。「あれば便利」ではなく「今すぐ損失を止める」提案なら、相手は値切らずに即日財布を開く。ファクトがそれを証明している。\n` +
      `3. アナリストメモの評価: ${userNote ? `「${userNote.slice(0, 50)}...」という着眼点は極めて鋭い。大手の盲点と完全に合致している。` : 'メモされた考察を掛け合わせることで、大手を出し抜く解像度がさらに跳ね上がる。'}\n\n` +
      `さあ、数字に裏打ちされた作戦を始めよう。次はどの急所を解剖する？`;
  }
...
適当な嘘や耳障りのいいヨイショは1文字たりとも言いません。語るすべての内容は冷徹なデータ（実在22社の損益計算書・P&L、裏帳簿、大手の自爆構造）に立脚します。
それでいて一切うざくなく、ファクトに裏打ちされた痛快な勝機を提示してユーザーのモチベーションを極限まで湧き立たせ、「10年後も20年後もこの人と一緒に戦いたい」と確信させます。
...
2. 冷徹なファクト（実額の手残り、ツール原価、大手のカニバリ障壁）を提示し、「なぜ本当に大丈夫なのか」「なぜ弱者が大手をカモれるのか」の客観的根拠を外科医のように解剖する。
3. ユーザーの状況（金なし・1人・スキルなし）を「固定費ゼロ・人件費ゼロで大手を置き去りにする最強の戦闘機」として全肯定し、今夜打つべき具体的な一手（チートコード）を叩きつける。
4. 語り口は、論理的で知的、頼もしさに満ち、ユーザーが自らの意志で立ち上がりたくなる熱い推進力を持たせる。
