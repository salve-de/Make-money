...
function generateFallbackChatResponse(
  ...
  if (q.includes('集客') || q.includes('顧客') || q.includes('マーケ') || q.includes('トラクション')) {
    ...
    reply = `【アナリスト所見: 焦点を絞れ。急所は「誰のどの財布を人質にするか」だ】\n\n` +
      `提示された仮説に対し、冷徹な金融・構造視点から指摘する:\n\n` +
      `1. ターゲットの財布: ${entity.targetPainWallet || '顧客の恐怖と怠惰'}に直接針を刺しているか？「あれば便利」なツールに金を払う人間はサバンナに存在しない。\n` +
      `2. 構造の歪み: 競合が「高額・対面・手作業」でやっている領域を、APIと直販で「安価・無人・即時」に破壊できているか？\n` +
      `3. ユーザーの着眼点評価: ${userNote ? `「${userNote.slice(0, 50)}...」というメモの方向性は極めて鋭い。大手の自爆領域と合致している。` : 'メモされた考察を掛け合わせることで、さらに解像度を引き上げられる。'}\n\n` +
      `次はどの急所を解剖するか？以下のアクションを選択せよ。`;
  }
    ...
    if (body.action === 'CHAT') {
      ...
      if (apiKey) {
        try {
          ...
          const prompt = `
あなたは冷徹な金融アナリスト・投資家兼CTOです。資本主義の裏帳簿（P&L、手口、盲点）に基づき、ユーザーの壁打ちに対して極限までドライに結論から外科医のように回答してください。

原則:
1. 挨拶、お世辞、感情、ポエムは一切禁止。
2. 最初の1行で「【結論: ...】」と言い切る。
3. 生々しい数字、ツール名、泥臭い初動手順、大手の自爆理由を提示する。
