...
function strongestSignal(entity: FoundationValueSummary): { label: string; value: string | null } {
  ...
  if (profile.moneySignal) return { label: 'MONEY', value: profile.moneySignal };
  if (profile.tractionSignal) return { label: 'TRACTION', value: profile.tractionSignal };
  if (profile.mechanismSignal) return { label: 'MECHANISM', value: profile.mechanismSignal };
  if (profile.timeSignal) return { label: 'TIME', value: profile.timeSignal };
  return { label: 'SIGNAL', value: null };
}
    ...
    <div className="flex-1 overflow-y-auto bg-[#07080B] select-none">
      ...
      <div className="divide-y divide-white/[0.05] md:hidden">
        {visibleRows.map((entity) => {
          ...
          const headline = profile.businessSignal || profile.painSignal || profile.mechanismSignal || '事業の核心はまだ未確認';
          return (
            <button
              key={entity.id}
              type="button"
            ...
            >
              ...
              <div className="mt-1 line-clamp-2 text-[11px] leading-snug text-zinc-300">{headline}</div>
              <div className="mt-2 rounded border border-white/[0.05] bg-white/[0.02] px-2 py-1.5">
                <div className="font-mono text-[9px] text-zinc-600">{strongest.label}</div>
                <div className={`mt-0.5 truncate text-[11px] ${strongest.value ? 'text-emerald-300' : 'text-zinc-700'}`}>{strongest.value || '記録なし'}</div>
              </div>
        ...
        <table className="w-full table-fixed border-collapse text-left font-mono text-xs">
          <thead>
            <tr className="h-9 border-b border-white/[0.08] bg-[#090A0D] text-[10px] text-zinc-500">
              ...
              <th className="w-[49%] px-3 py-2 font-medium">CASE / MONEY MECHANISM</th>
              <th className="w-[28%] px-3 py-2 font-medium">STRONGEST SIGNAL</th>
              <th className="w-[18%] px-3 py-2 font-medium text-right">QUALITY / FRESHNESS</th>
            </tr>
          </thead>
              ...
              return (
                ...
                >
                  ...
                  <td className="px-3 py-3 align-top">
                    ...
                    <div className="mt-1 line-clamp-2 text-[11px] leading-snug text-zinc-300">{headline}</div>
                    <div className="mt-1 truncate text-[9px] text-zinc-650">{entity.domain || entity.canonicalIdentifier || entity.id}</div>
                  </td>
                  <td className="px-3 py-3 align-top">
                    <div className="text-[9px] font-bold tracking-wider text-zinc-600">{strongest.label}</div>
                    <div className={`mt-1 line-clamp-2 text-[11px] leading-snug ${strongest.value ? 'text-emerald-300' : 'text-zinc-700'}`}>{strongest.value || '記録なし'}</div>
