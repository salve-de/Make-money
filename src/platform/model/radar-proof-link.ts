...
/**
 * Radar copy may retain old short IDs, but navigation must only use an ID that
 * actually exists in the current ledger. Prefix fallback is accepted only when
 * it resolves to exactly one entity; ambiguity fails closed.
 */
export function resolveRadarProofEntityId(
  ...
  const prefix = LEGACY_PROOF_PREFIXES[requested] ?? requested;
