...
/**
 * Read a byte range from an existing object. This is only a read optimization
 * for locating records inside immutable research bundles; it never writes or
 * changes the R2 storage layout.
 */
...
/**
 * Existing Foundation prefixes are read through the same R2 backend resolver
 * used by the immutable writer. This is a read-only listing operation; it
 * does not create a new object, index, or storage layout.
 */
  ...
  if (backend.kind === 'binding') {
    try {
      ...
      const objects = Array.isArray(raw?.objects)
        ? raw.objects.map(normalizeListObject).filter((item): item is R2ListObject => Boolean(item))
        : [];
      return {
        objects,
