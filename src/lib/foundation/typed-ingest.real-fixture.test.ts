...
describe('real scheduled Web ChatGPT typed sidecar fixture', () => {
  it('accepts the exact immutable bytes and preserves structured semantics losslessly', () => {
    ...
    expect(reported?.entity_ids).toEqual([
      'ent_organization_a1964fb04b1ac2226973',
    ]);
    const readable = readableObservationText(reported?.text as string);
    expect(readable).toContain('special_dividend.aggregate_amount=58350533');
    expect(readable).toContain('exchange_ratio_adjustment.spinco_cash_distribution_current_estimate=159000000');
    expect(readableObservationText(transport?.text as string)).toBeNull();

    const structure = observations.find(
      (item) => item.observation_id === 'obs_a6f30f25448da757a28dbf2a',
    );
    expect(structure?.entity_ids).toEqual(expect.arrayContaining([
      'ent_organization_1cebfd5fb60308398beb',
      'ent_organization_6152c81e5a2f6a67c0bc',
