/** Intake is permissive; completion requires a separately reviewable inventory. */
...
export function reconcileCollection(bundle: Row, required: readonly string[], productFields: readonly string[] = []) {
  const gaps: string[] = [];
  const audits = rows(bundle.observations).filter(x => x.kind === 'collection_audit.v1');
  if (audits.length !== 1) return { status: 'REVIEW_REQUIRED' as const, gaps: ['Exactly one collection_audit.v1 inventory required'] };
  ...
  const resolveRefs = (row: Row, label: string) => {
    ...
    for (const ref of refs) {
      if (!nonempty(ref) || !/^(entities|claims|metrics|money_signals|events|relationships|derived|observations)\/\d+$/.test(ref)) { gaps.push(label + ': invalid reference'); continue; }
      const record = pointer(bundle, '/' + ref);
      if (!record || record === audit) gaps.push(label + ': dangling/self reference');
      // An identity is never evidence that profit/cost/customer/stack facts were captured.
      if (ref.startsWith('entities/') && /^(requirement:|product:)/.test(label) && !/identity|name|domain|founder|[.:]id$|[.:]url$/.test(label)) gaps.push(label + ': identity-only reference');
    }
  };
  const checkRequirements = (items: unknown, expected: readonly string[], prefix: string) => {
    ...
    for (const row of records) {
      ...
      if (row.status === 'captured') resolveRefs(row, prefix + row.id);
        ...
        if (!nonempty(row.reason) || !Array.isArray(row.attempts) || !row.attempts.length || !row.attempts.every(nonempty)) gaps.push(prefix + row.id + ': unavailable without search log');
  ...
  checkRequirements(audit.product_fields, productFields, 'product:');
  const evidence = rows(bundle.evidence), reviews = rows(audit.sources);
  const seenSources = new Set<string>();
  for (const source of reviews) {
    if (!nonempty(source.evidence_id) || seenSources.has(source.evidence_id) || !evidence.some(x => x.evidence_id === source.evidence_id)) { gaps.push('Invalid/duplicate source review'); continue; }
    seenSources.add(source.evidence_id);
    if (source.status !== 'reviewed' || !nonempty(source.reviewed_at) || !Array.isArray(source.sections) || !source.sections.length || !source.sections.every(nonempty)) gaps.push(source.evidence_id + ': source not fully inventoried');
    ...
    for (const item of inventory) {
      ...
      if (item.status === 'excluded') {
        if (!nonempty(item.reason) || !['unrelated', 'rights_restricted', 'duplicate'].includes(String(item.exclusion_type))) gaps.push(label + ': unexplained exclusion');
      ...
      for (const check of checks) {
        ...
        if (!nonempty(check.pointer) || !refs.some(ref => nonempty(ref) && check.pointer !== '/' + ref && String(check.pointer).startsWith('/' + ref + '/'))) gaps.push(label + ': check outside retained record');
        if (value === undefined || !Object.hasOwn(check, 'equals') || JSON.stringify(value) !== JSON.stringify(check.equals)) gaps.push(label + ': retained value differs from inventory');
      }
    }
  }
