...
## Current verification authority (2026-09-12)

The older snapshots above retain their original counts and warnings as historical evidence. The current source tree is stricter: `pnpm lint` runs with `--max-warnings=0` and passes with zero warnings; `pnpm typecheck`, `pnpm test` (Vitest274 + Foundation11 + architecture8 + recovery6), `pnpm build`, `pnpm workers:build`, `pnpm deploy:preflight`, `pnpm audit --prod`, and `pnpm test:e2e` (Chromium25) all pass on the current HEAD. The Company Inspector is a feature Public API with explicit fixed composition and a typed dynamic evidence deck; plain J/K navigation is removed and only `⌘K` search plus `Escape` close remain. Production D1 migrations 0004/0005 are applied and read back, with zero application rows; the backup and limits are recorded in [RECOVERY.md](architecture/RECOVERY.md). GitHub Ruleset `22892592` is active for main, but the current local HEAD is not yet published and no current-HEAD GitHub CI run is claimed.

## Current authoritative verification (2026-09-12)

The snapshots above retain their original commit IDs and counts for audit history. They are not the current release state. The current branch is `codex/reliability-boundaries`; read the latest Git history and GitHub head before making a release decision.

The current local source passed `pnpm lint` with zero warnings, `pnpm typecheck`, `pnpm test` (Vitest 292, Foundation 11, architecture 11, Recovery 6), `pnpm build`, `pnpm workers:build`, `pnpm deploy:preflight`, `pnpm audit --prod`, and `pnpm test:e2e` (26 Chromium tests). In a real browser using the local fallback path, Photo AI showed `未確認` financial values and Evidence, lowercase `j`/`k` did not change the selected company, `Escape` closed the inspector, the list reopened it, and the visible close button closed it; console warnings/errors were empty. This browser run does not prove live Foundation R2 binding.

GitHub ruleset `main-quality-gates` is active on `refs/heads/main` and requires `lint`, `typecheck`, `unit test`, `build`, and `E2E smoke`, with no bypass actors. The feature branch has not been merged. Production Worker cutover, live Stripe roundtrip, scheduled backup/RPO/RTO operation, and production restore remain unverified. The current serving index is 121 records (`VERIFIED 7`, `ESTIMATED 35`, `REPORTED 69`, `POST_MORTEM 3`, `UNAVAILABLE 7`); 135-record and 324-test statements elsewhere are historical and are not current evidence.
