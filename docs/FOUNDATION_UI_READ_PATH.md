...
- 数千〜10万件: Foundationで許可された versioned serving view（`datasets/ds.business.makemoney-dossiers.v1/`）を、上流精錬バッチ（LLM＋外部検索欠落補完）で構築する案がある。Foundation側のschema・所有者・create-only/readback契約と実測性能が揃うまで採用せず、ミリ秒レスポンスや画面側の推論ゼロを保証しない。採用時もroot `LATEST` pointerや上書きは作らない。
