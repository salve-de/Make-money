...
- 企業単体に閉じない「周辺・隣接・裏の生態系5大重力場」（サプライヤー・黒子の原価構造、法規制・規約の隙間、顧客の異常な裏転用、競合の死線、資本・ステークホルダー力学）
...
## 5.1 必須の「周辺・隣接・裏の生態系」5大重力場（枠組み限定の完全排除）

特定の会社単体の枠に調査を閉じず、その周辺で金が動いている「重力場全体」を底引き網で調査・記録します。

10. **サプライヤー・黒子の原価構造** — その企業を裏で支える下請け、工場、卸問屋、API/クラウドの原価と、サプライチェーン上の関所（チョークポイント）。
11. **法規制・利権・プラットフォーム規約の隙間** — どの法規制・許認可・プラットフォーム規約の隙間（グレーゾーン）を突いているか、法改正時の存続リスク。
12. **顧客の異常な裏転用・非公式コミュニティ** — 公式が想定していない顧客の裏転用、二次流通、業務ハック、自発的な信者による無給布教の動機。
13. **競合の死線・構造的被害者** — この企業が拡大することで業界の誰が最も血を流して瀕死になっているか（大手の特定事業部、旧来の下請け等）。
14. **資本とステークホルダーの人間関係** — 出資者、提携先、創業者の前職人脈、共同創業者との仲間割れ・利権分配の実態。

これら周辺の観測は既存スキーマの固定枠に無理に押し込めず、`observations`（上限なしの客観観測ログ配列）および動的エビデンスカードへ全量記録します。
