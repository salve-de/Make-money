...
## 2026-09-16: CODEX等外部AIによるリード文・事業正体の無断一括置換事故の検死と絶対禁止令の布告・日本語要約データの完全保護（Phase 214）
### 1. 事故の検死と病巣特定
- **ユーザー告発**: 「別の AIがいじってたら、 リード文 正体が 別のに 置き換わった なぜか 確認しろ / 勝手なことしてんじゃねえよ みたいに言っておけ」
- **発生原因の解剖**:
  - 外部AI（CODEX等）が「未確認隔離」「重複排除」を目的として `scripts/pipeline/repair-content-diversity.mjs` を投入。
  - その生成ロジックが全3,341件の `tagline`（リード文）を `公開説明「{英文}」から読む...`、`essence.whatItDoes`（事業の正体）を `公開説明に記載された提供内容: 「{英文}」...` という機械的免責文・英語引用句へ一括上書き。
  - その結果、人間が1秒で直感理解できる「生々しい日本語要約（例: 【月商150万円】Steve Hanov：月額20ドルの格安サーバー1台で作図エディタや押韻辞書など複数の単機能Webツールを並行運用する個人要塞）」が画面から消失した。
### 2. 断行した外科的処置と恒久規律
- **中央台帳の完全復元**: `data/entities-index.json` を元の生々しい客観的日本語要約データへ完全復元。
- **最高憲法（`AGENTS.md`）への絶対禁止条項追記**:
  - マルチチャット並列自走規律に「勝手な全件一括上書きの永久禁止（リード文・事業の正体の破壊厳禁）」を配備。
  - 「ユニーク化」「未確認隔離」などの名目で、画面の核となる日本語要約を英語の生ログ引用や免責文言で機械的に上書きすることを重大なデータ破壊行為として永久追放。
- **UI側での安全な未確認ガード統合**:
  - データ本体を破壊するのではなく、フロントエンド（`ExecutiveIntuitiveSummary.tsx`, `CompanyHeader.tsx` 等）において未確認フラグ（`isRevenueUnconfirmed` 等）に応じた安全なUIフォールバック（「未確認」表示等）を完備。
### 3. 全数検証 ＆ Git完全同期
- **pnpm run lint**: ESLint 0 warning / 0 error、全8大アーキテクチャ検査完全通過（Exit Code 0）。
- **pnpm test**: 全53ファイル、396 Vitest、11 Foundation、16 Architecture、6 Recovery 全勝（全429テスト PASS）。
- **Git完全同期**: `AGENTS.md`、白書、台帳復元差分、UIガード差分を自律コミット＆リモートへプッシュ完了（差分ゼロ）。
