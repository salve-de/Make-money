...
### データ収集エージェントの鉄則（限定の完全排除 ＆ 周辺・隣接生態系の全量吸い上げ）
1. **あらゆる前提・限定の完全排除**: SaaS、製造、物販、店舗、メディア、下請け、大企業、個人、表、裏など、いかなる業種・規模・形態・場所の限定も一切禁止する。資本主義で現実に金が動いている（または尽きて死んだ）事実は全て収集対象である。
2. **企業単体に閉じない「周辺・隣接・裏の生態系5大重力場」の全量吸い上げ**: 単体の調査に限定せず、その企業を取り巻く①サプライヤー・黒子の原価構造、②法規制・規約の隙間、③顧客の異常な裏転用、④競合の死線、⑤資本・利権分配の人間関係を枠組みなしで柔軟に吸い上げる。
3. **収集項目と足切り条件の完全分離**: 人数・粗利・原価・URL・創業者・ツール・周辺生態系等は**当然すべて徹底調査して記録する**。しかし、**いかなる項目の有無や数値も【収集の足切り条件】には絶対にしない**。
4. **事実と手口（裏帳簿）の全量記録**: 綺麗事の定型文を排し、誰から・どんな手口で・いくら抜いたか（またはどこで死んだか）の客観的ログを記録せよ。
5. **オーナー（ユーザー）が真に求めている5大最高意志（設計思想）**:
...
### 【探索範囲：全方位探索 ＋ 5大深層盲点チェック（必須巡回）】
公式発表、有価証券報告書、決算説明会、技術ブログ、SNS、業界レポート等による**全方位探索を大前提**とし、その上でAIが表面のPR文（「顧客の笑顔のために」）だけで満足して見落としがちな以下の5大深層情報**も**必ず掘り尽くせ：
...
5. **ゼロ足切り ＆ 欠損誠実受入原則（手堅い商売の全量受入 ＆ 事業実体基準）**:
   - **避けるべき「ゴミ」の境界**: 落とすべきなのは「地味な商売」ではない。**「売上$0の実験場、月数千円のお小遣い稼ぎ、趣味ブログ、学生の工作など事業実体のないノイズ」のみを排除**せよ。
   - **手堅い商売の全量拾い上げ（事業として自立している実体）**: 個人開発なら**「個人が飯を食えている・自立している規模（月商数十万〜100万円以上等）」**、企業なら持続的な利益が出ている実業（配管清掃、一人SaaS、町工場等）は、派手なズルがなくても100%全量拾い上げよ。
   - **欠損の誠実受容（1つ抜けているだけで破棄するな）**: 個人開発や小規模企業では、原価の内訳や正確なツール代が非公開で取れないケースがあって当然である。「一部の数字が取れない」「URLがない」からと収集を諦めたり捨てたりすることは厳禁。**取れない項目は `null` や `UNKNOWN`、空配列のままで堂々と記録せよ。**
   - **水増しテンプレ・架空作文の永久禁止**: 項目を埋めるため・文字数を稼ぐために抽象的な作文を捏造することは最大の禁忌である。手元にある生々しい客観事実だけを記録せよ。
   - **社内造語・スラングの完全排除**: 「サバンナOS」「略奪転用」等の社内メタ用語を表文に混入させず、客観的・学術的・平易な日本語で記録せよ。

### 1.1 外部AI過去重大やらかし事故リスト ＆ 厳格再発防止条項（二度と繰り返すな）
過去に外部AI（ChatGPT等）が生成したデータにおいて、以下の致命的バグ・誤記・スキーマ不整合が発生し、人手・外科手術によるクレンジングを余儀なくされた。今後収集を行う全てのAIは、以下の地雷を絶対に踏むな：

1. **為替レート逆数掛けバグ（数倍〜数十倍の計算狂い）**:
   - **事故例**: TSMCで新台湾ドル（NTD/JPY = 4.71477）を誤って割り算（1/4.71477掛け）し、年商13兆円の企業を約6,000億円と出力した。
   - **防止策**: 外貨から日本円への換算は必ず「現地通貨額 × 為替レート＝日本円」である。逆数で割るな。
2. **タグラインのケタ・単位誤爆（100倍誤記）**:
   - **事故例**: PayPalで年商4.96兆円であるにもかかわらず、タグラインで「年商497兆円」と表記した。
   - **防止策**: 本文・タグライン・P&Lの数値（億円・兆円）は必ず突き合わせ、ケタ・単位の矛盾をゼロにせよ。
3. **既存ID・名称・Tickerの衝突**:
   - **事故例**: 37signalsのTickerを`BASECAMP`（別エンティティと衝突）、WeWorkで既存の検死レコードと同一名称で衝突した。
   - **防止策**: `id`, `name`, `ticker` はシステム全体で完全一意でなければならない。同一企業の別切り口（検死版等）を記録する場合は `WeWork (2019破綻前夜検死)` のように名称・IDを明確に区別せよ。
4. **未公開（UNAVAILABLE）時の未確認フラグ欠落**:
   - **事故例**: Mailchimp等の非公開企業で売上未公開であるにもかかわらず、未確認フラグ（`isRevenueUnconfirmed: true`等）を付けずにバリデーションを落とした。
   - **防止策**: 財務未公開時は `financialStatus: 'UNAVAILABLE'` とし、`isRevenueUnconfirmed: true`, `isMarginUnconfirmed: true` を漏れなく付与せよ。
5. **スキーマ必須型の欠落**:
   - **事故例**: `operations` 内の `weeklyHours`, `initialCapitalRequired`, `automationLevel` や、`evidenceCards` 内の `evidenceStatus`（`REPORTED` / `POST_MORTEM`）、`observations` と `observationsStream` の分離が漏れて型エラーになった。
   - **防止策**: 本仕様書の第4章・第5章の完全体JSONスキーマを1文字たりとも漏らさず満たせ。
6. **取れなかった情報へのモック捏造・固定文字列流し込み**:
   - **事故例**: スクレイピングで創業者やタイトルが取れなかった際、三項演算子のフォールバックで架空のモック文字列を捏造流し込みした。
   - **防止策**: 取れなかったものは堂々と `UNKNOWN`、空配列、未判定にせよ。確定Factの偽装は最大の禁忌である。
7. **社内造語・スラングの外部データ本文への混入**:
   - **事故例**: 「サバンナOS」「略奪配管」といった本プロジェクトの開発用メタ言語を、企業解説やタグラインの表文にそのまま書き込んでリントで弾かれた。
   - **防止策**: データ本文は客観的・学術的・平易な日本語（「保身恐怖」「独占利権」「スイッチングコスト」「解約障壁」等）で記述せよ。
8. **#01〜#04カード非表示・1行ポツン手抜き事故（essence欠落・証拠カード不足・見出し記号欠落）**:
   - **事故例**: `essence` を空欄にし、`evidenceCards` を2枚でサボり、`strategy`（盲点・堀・弱点）を【見出し】のない1行ポツンで出力したため、画面右側のセクションが非表示やスカスカになった。
   - **防止策**: `essence`（whatIsIt, whoPays, solvedProblem）はUI表示の必須骨格として簡潔かつ生々しい事実を記述（空欄絶対禁止）。`strategy` の各解説は `【パンチライン見出し】詳細解説本文` の階層構造で記述せよ（文字数ノルマではなく、見出しと本文を分ける構造化が目的）。
9. **英語生ログ丸投げ ＆ 後工程AIによる一括免責置換データ破壊事故（超重大インシデント INCIDENT-2026-09-16-01）**:
   - **事故例**: 海外サイト（Indie Hackers等）から収集する際、英語の1行タグラインを収集エージェントが日本語化・事業分析せずそのまま台帳へ流し込み、後工程の外部AI（CODEX等）が全3,341件の `tagline` や `essence.whatItDoes` を「公開説明『...』から読む...」「公開説明に記載された提供内容:... 実装詳細は未確認」という機械的免責文言・英語引用句で一括上書きして日本語の裏帳簿を全滅させた。
   - **防止策**: 収集段階でその場で「誰から・どうやって現金を抜いているか」を確定した客観的日本語で1行に要約してJSONに焼き込め。英語丸投げや免責プレフィックスはインジェスト関所（`check-ingest-quality.mjs`）で即座に一発Rejectされる。また台帳への一括置換スクリプト実行を永久禁止とする。
...
| 4 | `tagline` | `string` | 日本語1行 | サバンナOS直撃のタグライン（痛みの財布＋手口＋数字） [必須・確定日本語要約義務。英語丸投げ・免責テンプレは一発Reject] |
...
### 2.1 周辺・隣接・裏の生態系5大重力場の無制限吸収（`observations` ＆ `evidenceCards`）

上記基本10属性は最低限の骨格であり、収集の上限（天井）ではない。企業を取り巻く周辺・隣接情報（①サプライヤー・黒子の原価構造、②法規制・規約の隙間、③顧客の異常な裏転用、④競合の死線、⑤資本・利権力学）は、以下のオープンスキーマ領域へ上限なく柔軟に全量記録せよ：

1. **`observations` 配列（客観観測ログ・上限なし）**:
   - スキーマの固定枠に収まらない周辺事実、業界の悪習、現場の裏ワザ、サプライヤーとの力関係等は、`observations` に客観ログ（日時、情報源、観測内容）として何十件でも上限なく追記せよ。
2. **`evidenceCards`（動的エビデンスカードの拡張）**:
   - 型通りのカードに加え、「サプライヤーの闇」「規制ハック」「顧客の異常行動」など固有の切り口のカードを無制限に動的追加せよ。
3. **生原本（Raw CAS）の完全保全**:
   - 取得した生バイト列（HTML/PDF）は `foundation-raw` にSHA-256で完全不変に保存されるため、将来の新規プロジェクトや分析でいかなる周辺情報も再抽出可能である。
...
企業固有の特異点事実を最低1枚以上（うち勝ち組は `LOOT_BLUEPRINT`、地雷組は `FATAL_BLEED` 必須）配備せよ。

1. **`LOOT_BLUEPRINT`（略奪転用方程式）[勝ち組必須]**:
...
```json
{
  ...
  "essence": {
    "whatItDoes": "大手が肥大化させた機能を1画面に凝縮し、キーボードだけで操作が瞬時に完了する高速軽量ツール。",
    "targetCustomer": "動作の遅い大手ツールに毎月高額な利用料を払わされストレスを抱える現場担当者や個人開発者。",
    "painRelief": "過剰な多機能設定による時間浪費と、不要な固定費負担の苦痛。"
  }
}
```
...
```json
{
  ...
  "essence": {
    "whatItDoes": "ハリウッド級の短尺動画をスマホ専用に制作・配信したが、半年で1,800億円を全額焼失し即死した地雷検死標本。",
    "targetCustomer": "移動中の隙間時間に映画並みのコンテンツを見たいと仮定されたスマホユーザー層、および出資した大手スタジオ群。",
    "painRelief": "（仮説上設定されたが実態ゼロだった）隙間時間の退屈と、低品質な無料動画への不満。"
  }
}
```
...
## 6. マルチチャット並列自走規律（排他予約 ＆ 隔離保存）

就寝中や複数チャット（別AI）を立ち上げて同時に事例を収集・精錬させる場合は、以下の排他ロックプロトコルを100%遵守せよ：
