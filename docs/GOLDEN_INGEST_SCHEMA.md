...
5. **コンテンツ完全密度基準（Fail-Closed Gate：薄いデータの即時排撃）**:
   - `observations` は最低4件以上（確定財務、初動集客、業界死角、定着仕組み、運用インフラ等の事実ログ）を完全収録せよ（「メモ1行」等の手抜きは即座にバリデーション拒絶される）。
   - エビデンスカードは3枚すべてにおいて `details` に最低3箇条以上の構造的詳細を記述せよ（空配列・1行は不可）。
   - 初動集客手順（`executionChecklist`）は最低3ステップ以上を記述せよ。
   - 事業概要（`essence`）の各項目は最低40文字以上の重厚な解説を記述せよ。
   - 意味不明な社内造語・スラングは完全排除し、客観的・学術的・平易な日本語で記録せよ。
...
8. **#01〜#04カード非表示・1行ポツン手抜き事故（essence欠落・証拠カード不足・見出し記号欠落）**:
   ...
   - **防止策**: `essence`（whatIsIt, whoPays, solvedProblem）は各40文字以上で完全記述、`evidenceCards` は最低3枚（ウォーターフォール、サバンナOS急所、4マス指標グリッド）を配備、`strategy` は必ず `【パンチライン見出し】詳細解説本文` の階層構造で記述せよ。
