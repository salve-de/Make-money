...
Evidence-only corrections keep the existing `make-money-view.v2` entity document and may add `excluded_source_run_ids` / `excluded_evidence_ids` control metadata. The operator repair verifies exact immutable original/corrected bundle hashes, resolves all other contributing runs from existing projection-progress records, and CAS-rebuilds the entity view without overwriting canonical records. Subsequent updated projectors preserve the exclusions. A full view-loss recovery must reapply the correction from the immutable corrected bundle, Journal lineage and repair receipt; deployment of older writers is not covered by the new guard. See `AUTO_PUBLISH_TO_UI.md` for dry-run, failure and history bounds.
