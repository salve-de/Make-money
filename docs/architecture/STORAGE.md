...
5. **未加工英語生ログをLayer 3（Serving View）へ直結流し込みしない（超重大インシデント INCIDENT-2026-09-16-01 教訓）**:
   - Layer 1（Raw）の英語生ログを、日本語要約（精錬）を経ずにそのままLayer 3の台帳（`tagline`, `essence.whatItDoes`）へ放り込んではならない。
   - 後工程で外部AIが「免責文」「英語引用」で台帳を一括上書きする事故を防ぐため、Layer 3のDossier生成時は必ず「生きた日本語要約」をインジェスト時に確定結合（焼き込み）せよ。台帳全件を一括置換するスクリプトの実行は物理移動ゼロ原則・冪等性原則違反として永久に封印する。
