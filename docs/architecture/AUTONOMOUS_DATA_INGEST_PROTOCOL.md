# 全プロジェクト共通：完全自律データ収集 ＆ 100年R2保存プロトコル (Autonomous Ingest & 100-Year R2 Protocol)

> **最高運用契約**: ユーザーは長文プロンプトや追加条件を絶対に書かない。
> 「AGENTS.md見てデータ集めて」の一言で、全AI（Antigravity, Claude, ChatGPT, Cursor）は一切の追加質問・手戻りなしに完全自律稼働せよ。
> 本プロトコルは、Make-Money、Investrader、universal-foundation、および今後立ち上がる全プロジェクトにそのまま適用可能な【最高標準契約】である。

---

## 0. プロジェクトの北極星とデータ収集の理念（なぜ集めるのか）

すべてのデータ収集エージェントは、作業を開始する前に以下の理念と存在意義を完全に脳へ焼き付けよ。単なる「スキーマの穴埋め作業員」として動くことは即時失格である。

### 【プロジェクトの北極星】
> **「世に溢れる『努力・理念・綺麗事』という欺瞞の煙幕を完全に焼き払い、世界中の『生々しい金儲けの事実と手口（資本主義の裏帳簿）』を冷徹に白日の下に晒し続けることで、隷属と理不尽から抜け出そうと抗う全人類に『これを持たずに動くのは自殺行為だ』と確信させる【絶対の武器（資本主義のデフォルトOS）】となり、あらゆる事業・起業・投資が始まる『最初の起点（マネーの交差点）』として君臨し、その巨大な引力場（情報の関所）で全方位から富を抜き続けること。」**

### 【最高視座：我々が最もメタ的に収集したいもの】
我々が収集したいのは特定の企業データではなく、綺麗事（努力・理念）を完全に剥ぎ取った後に残る**【資本主義における『富の不可逆な移転メカニズム（物理法則）』のログ】**そのものである：
1. **「弱点の力学」（なぜ金が動いたか）**: 人間や組織が理性を失って財布を開かざるを得なかったサバンナOSの急所（保身恐怖、極限の怠惰、虚栄心、解約不能の監禁、独占利権）。
2. **「配管の力学」（どうやって現金を吸い上げたか）**: 労働から解放され、大手の自爆（カニバリズム）、プラットフォーム規約の盲点、他人の欲望、前金総取りを燃料にして自動で現金を吸い上げた構造的レバレッジ（関所）。
3. **「現金の力学」（結果として通帳に何が起きたか）**: 見栄の年商ではなく、原価・手数料・税を全て引いた後に「創業者個人の通帳に実際に残った現金実額（真の手残り）」、または幻想が崩壊して即死した「致死出血点」。

### 【データ収集エージェントが遵守すべき5大メタ理念（限定の完全排除 ＆ 周辺生態系の全量吸い上げ）】
1. **全方位・無限定の底引き網（明暗・表裏を問わず全てが対象）**:
   - 「街灯の下（表舞台のSaaS）」ばかり探すのも誤りだが、「暗闇（下請けやニッチ）にしかない」と決めつけて限定するのも同等に致命的な誤りである。
   - 表舞台の巨大プラットフォーム・上場SaaSから、町工場、D2C物販、クリニック店舗、下請け、匿名インフラ、そして数千億の調達死から小規模撤退まで、**「表か裏か」「明か暗か」といった場所や属性による限定は一切禁止**する。資本主義で現実に金が動いた（または尽きて消滅した）現場はすべて等しく対象である。
2. **企業単体に閉じない「周辺・隣接・裏の生態系5大重力場」の全量吸い上げ**:
   - 調査対象を企業単体の枠に閉じず、その周囲で金が動いている重力場全体（①サプライヤー・黒子の原価構造、②法規制・規約の隙間、③顧客の異常な裏転用、④競合の死線、⑤資本・利権分配力学）を底引き網で吸い上げよ。枠に収まらないファクトは `observations` や動的エビデンスへ無制限に記録せよ。
3. **プロクラステスの鉄のベッドの完全粉砕**:
   - ギリシャ神話の山賊プロクラステスの如く、「粗利80%」「1人」「URL必須」「下請け」といった特定のビジネスモデルや数値枠（鉄のベッド）を用意し、現実の手足を切り落とすな。
4. **収集項目（全量取得）と足切り条件（完全排除）の絶対分離（可逆的ELT ＆ 欠損誠実受容）**:
   - **収集項目（データ）**: 人数（チームサイズ）、粗利率、営業利益率、URL、創業者、ツール構成、周辺生態系など、**必要なデータは当然すべて徹底調査し、全量収集して記録する**。
   - **足切り条件（完全排除）**: しかし、**いかなる数値や項目の欠落も【収集の足切り条件（フィルター）】には絶対にしない**。
   - **手堅い商売の全量受入**: 派手なズルやハックがなくても、「地元の工場を地道に回ってリピート率95%で営業利益30%抜いている配管清掃会社」「ニッチな帳票ツールを一人でメンテして月商300万をほぼ利益にしている個人SaaS」「職人技術や特許、立地で競合を無力化している町工場」等、**現実に利益が出ている手堅い構造は100%全量拾い上げよ**。
   - **避けるべき「ゴミ」の境界**: 落とすべきなのは「地味な商売」ではない。**「売上$0の実験場、月数千円のお小遣い稼ぎ、趣味ブログ、学生の工作など事業実体のないノイズ」のみを排除**せよ。個人開発なら**「個人が飯を食えている・自立している規模（月商数十万〜100万円以上等）」**、企業なら持続的な利益が出ている実体を柔軟に拾い上げよ。
   - **欠損の誠実受容**: 原価の内訳や正確なツール代が非公開なら、`null` や `UNKNOWN`、空配列のままで堂々と記録せよ。項目を埋めるための架空の作文やテンプレ水増しは永久に禁止する。
5. **あらゆる特異点・事実ログの独占**:
   - 資本主義のアルファは特定の型からは出ない。表の巨人の自爆構造から裏の集金配管まで、あらゆる金儲けの事実ログこそが我々の独占資産である。

### 【探索範囲：全方位探索 ＋ 5大深層盲点チェック（必須巡回）】
公式発表、有価証券報告書、決算説明会、技術ブログ、SNS、業界レポート等による**全方位探索を大前提**とし、その上でAIが表面のPR文（「顧客の笑顔のために」）だけで満足して見落としがちな以下の5大深層情報**も**必ず掘り尽くせ：
1. **創業者個人の過去ログ（X, Reddit, IndieHackers, HackerNews）**: 会社の公式発表ではなく、創業者個人のリプ欄やAMAに転がっている「初動の泥臭いズル」「Stripe管理画面スクショ」「初期P&Lのポロリ告白」。
2. **Wayback Machine（初期魚拓）**: 現在の洗練されたサイトではなく、創業初期の「ショボい初期機能」「買い切り時代の生々しい価格」「初期の泥臭いタグライン」。
3. **求人票（Job Description / Wantedly / Lever等）**: 採用要項に赤裸々に書かれた「社内で実際に稼働しているツール群（AWS, Stripe, Snowflake等）」「現場の真の課題・ボトルネック」「本当の職種別組織比率」。
4. **裏アフィリエイト・キックバック配管（フッター, ASP, Rewardful等）**: 表のLPには出ない「売上の30〜50%永久キックバック」「紹介報酬」という、広告費ゼロで広げている真の集客エンジン。
5. **解約怨嗟・悪評（Trustpilot, Reddit, G2等）**: 公式のサクラ事例ではなく、「解約できない」「データが人質」「他社に移れず泣く泣く払っている」という、高収益を支える理不尽な監禁（スイッチングコスト）の実態。

---

## Ⅰ. 0クリック自走原則（Zero-Effort Execution）

1. **一言発動（One-Phrase Trigger）**:
   - ユーザーが「AGENTS.md見てデータ集めて」「〇〇のデータ集めてR2に入れて」と一言口にした瞬間、AIは一切の質問（「どの企業にしますか？」「どう保存しますか？」等）を返さず、直ちに底引き網調査とR2保存を開始せよ。
2. **優等生AI病の完全粉砕**:
   - 「非公開だから取れない」は即失格。単価×推定アクティブ顧客数×業界原価率（Stripe 2.9%+$0.30、サーバー代、API代等）からP&Lウォーターフォールを科学的に逆算し、`originType: 'estimated'` と計算式を明記せよ。
3. **道徳・美談の完全排除**:
   - 創業者の綺麗事（「価値ある発信でファンを増やした」）を焼き払い、Reddit自演、コールドDM爆撃、ToS隙間ハック等の泥臭い初動突破事実ログを抉り出せ。
4. **【最重要】収集 ＆ UI表示の完全並行原則（後回し・分離の絶対禁止）**:
   - **「データを溜めるだけ溜めて、画面UIの表示は後回しにする」ことは厳禁である。**
   - トップ画面（`src/app/page.tsx`）は目録（`data/entities-index.json`）を直読して金融端末へ流し込む。
   - 【底引き網調査 ➔ P&L逆算 ➔ 完全体JSON精錬 ➔ R2保存 ➔ 目録（entities-index.json）登録 ➔ ブラウザUI（一覧テーブル＆詳細インスペクター）で崩れなく0.01秒で描画されることを実機確認】までを1セットとして並行稼働せよ。
5. **【最重要・超重大インシデント教訓】英語生ログ丸投げの完全禁止 ＆ 収集時確定日本語要約の絶対義務（INCIDENT-2026-09-16-01）**:
   - 海外サイト（Indie Hackers, eBiz Facts等）から収集する際、英語の生ログ（例: `Track daily prices...`）を未加工のまま `tagline` や `essence.whatItDoes` に放り込むことを厳禁とする。
   - データを拾った瞬間（インジェスト時）に、その場で「誰から・どうやって現金を抜いているか」を確定した客観的日本語で1行に要約してJSONに焼き込め。
   - 後工程の外部AIが「未確認隔離」「重複排除」を口実にして台帳（`entities-index.json`）を機械的免責文（「公開説明に記載された…」「実装詳細は未確認」）で一括上書きする事故を防ぐため、英語丸投げや免責プレフィックスが含まれるJSONはインジェスト関所（`check-ingest-quality.mjs`）で即座に一発Rejectされる。中央台帳の一括置換スクリプト実行は永久に禁止する。


---

## Ⅱ. 3大収集モード切り替えトリガー（ユーザーの一言で即時分岐）

AIはユーザーの短い一言から意図を即座に判定し、以下の3モードのいずれかに自律分岐せよ（追加質問は厳禁）：

| ユーザーの指示（一言） | 実行モード | 構成比 | 収集対象・必須要件 |
|:---|:---|:---:|:---|
| **「勝ちの方だけ集めて」**<br>「成功事例だけ」「黒字事例だけ」 | **① 勝ち組特化モード** | **10:0** | 業種・規模・利益率の数値による限定・足切りは一切禁止。物販・製造・SaaS・メディア・店舗等、資本主義の構造を突いて現金を抜いている勝者を柔軟に全量収集。<br>（必須: `LOOT_BLUEPRINT`, `THE_CRIME`） |
| **「地雷の方だけ集めて」**<br>**「負けの方だけ集めて」**<br>「大爆死事例だけ」「失敗企業集めて」 | **② 地雷検死解剖モード** | **0:10** | 業種・規模不問。巨額調達即死、CAC高騰炎上、規約変更死、固定費過多死などの失敗・撤退ログを柔軟に収集。<br>（必須: `financialStatus: 'POST_MORTEM'`, **`FATAL_BLEED`（死因出血解剖）[100%必須]**, `LOOT_BLUEPRINT`（回避配管）） |
| **「データ集めて」**<br>「AGENTS.md見てやって」<br>（※特に指定がない場合） | **③ 攻守混合デフォルト** | **7:3** | 勝ち組（約7〜8割）と地雷組（約2〜3割）を柔軟に底引き網収集。生存者バイアスを粉砕し、損失回避本能を着火させる。 |

> **【最高鉄則：収集項目と足切り条件の完全分離】**:
> - **収集項目（データ）**: 人数（チームサイズ）、粗利率、営業利益率、URL、創業者、ツール構成など、**必要なデータは当然すべて徹底調査し、全量収集して記録する**。
> - **足切り条件（フィルター）**: しかし、**いかなる数値や項目の有無も【収集の足切り条件（フィルター）】には絶対にしない**。
> - 粗利が10%でも50%でも80%でも、社員が1人でも10人でも1万人でも、URLがあっても無くても、**「生々しい金儲けの事実と手口（裏帳簿）」または「失敗・撤退の事実ログ」があるものは、全て柔軟に拾い上げてデータを記録せよ**。

---

## Ⅲ. 基本10属性 ＆ 完全体JSON（柔軟収集・足切り完全禁止）

> **【限定・足切りの完全排除原則 ＆ 全項目必須の完全撤廃】**:
> **絶対に全ての項目が必要なわけではない。URLがなくたっていい。P&L内訳が取れなくたっていい。エビデンスカードが1枚しかなくたっていい。**
> 創業者の告白ポスト1本、初動の泥臭い集客手口1点、あるいは即死した死因ログ1行など、資本主義で金が動いた（または尽きた）生々しい事実が1つでもあれば、それだけで即座に拾い上げよ。
> 形式的な項目の欠落を理由にした足切り・収集破棄・テンプレ水増しは永久に禁止する。取れたファクトをありのまま記録し、ないものは `null` や `UNKNOWN` のままで堂々と保存・公開せよ。

1. `id`: 一意のエンティティID（`ent_<name>_<hash>`）[必須]
2. `ticker`: 4〜10文字のシンボル（例: `KEYENCE`, `GYM.SHRK`, `FAIL.BLEED`。なければIDから自動生成可）
3. `name`: 正式名称 [必須]
4. `tagline`: サバンナOS直撃の日本語1行（痛みの財布＋手口＋数字） [必須]
5. `sector`: 7大業種（`AI_AUTOMATION` / `NICHE_SAAS` / `MONOPOLY_MFG` / `CONTENT_MEDIA` / `PHYSICAL_ASSET` / `FINTECH_INFRA` / `LOCAL_SERVICES` 等の大枠判定）
6. `scale`: 規模区分（`SOLO` / `SMALL_TEAM` / `SCALEUP` / `ENTERPRISE` / `UNKNOWN`）
7. `founder`: 創業者・仕掛け人（**実名・組織名・匿名・不明いずれも可**）
8. `country`: 国コード（ISO 2文字。不明なら `JP` または `GLOBAL`）
9. `url`: 公式サイトURL（**任意・なし可**。サイト消滅、非Web、下請け、オフライン等の場合は `null` または省略）
10. `temporal`: 時系列・賞味期限インテリジェンス（創業年、初動獲得期、観測期、賞味期限判定等。取れる範囲で記録）

### P&L因数分解（円換算・Stripe手数料控除・創業者手残り）
すべての金額は円（JPY）で統一（1ドル=150円、1ポンド=190円換算）：
- `monthlyRevenue`（月商） / `cogs`（売上原価） / `grossProfit`（粗利益） / `grossMargin`（粗利率%）
- `operatingExpenses`: `serverAndApi`（推論・サーバー代）, `advertising`（広告費）, `subcontracting`（外注費）, `toolsAndSaaS`（ツール代）, `other`（その他）
- `operatingProfit`（営業利益） / `operatingMargin`（営業利益率%） / `estimatedAnnualNetProfit`（年間純利益）
- `financialStatus`: `VERIFIED` / `REPORTED` / `ESTIMATED` / `POST_MORTEM` / `UNAVAILABLE`

### 稼働ツール配管（`operations.toolStack`）
実際に確認できたツール群を月額原価（円）付きで列挙（Stripe, AWS, Cloudflare, Next.js等）。

---

## Ⅳ. 100年壊れないR2保存の3層メダリオン構造

R2ストレージは、人間の感情による物理フォルダ整理を永久に禁止し、以下の**「3層構造（物理移動ゼロ）」**で完全固定する：

```text
［Layer 1: 原物（Bronze / Raw）］ ── 完全不変（Write-Once, 削除・上書き禁止）
  └─ foundation-raw/blobs/sha256/<hash>              （Web魚拓、PDF原本、スクレイピングHTML）

［Layer 2: 保存票（Silver / Lake）］ ── 追記専用（Append-Only）
  └─ foundation-lake/journal-entry.v1/<id>.json      （上記Golden完全体JSON、事実ログ）

［Layer 3: 目録・提供ビュー（Gold / View）］ ── 全自動再生成可能（Derived）
  └─ entities-index.json / D1                        （単一目録側で1行JOINして画面へ描画）

【絶対不可侵境界】
  └─ universal/data-assets/financials/               （EDINET正本領域には一切触れない・書き込まない）
```

### 100年運用の4大鉄則
1. **原物を解析結果で置き換えない**: 原本は永久不変。訂正も新しい履歴として追記する。
2. **保存完了を先に宣言しない**: 不完全な取得・保留・失敗はその状態で保持する。
3. **同じ依頼を再送しても壊れない**: 不変ハッシュにより重複再送されても安全に同一結果へ収斂する（冪等性）。
4. **復元できることを実際に試す**: 上位のGoldやUIキャッシュが全損しても、原物と保存票からいつでも100%全自動再生成できる。

---

## Ⅴ. 他プロジェクト（Investrader, Foundation等）への導入手順

本プロトコルを他のリポジトリに展開する際は、以下の3ステップのみで完了する：

1. **AGENTS.mdの先頭に本プロトコルを宣言**:
   ```markdown
   > **2026-09-14 最高運用契約（ゼロプロンプト宣言）**: ユーザーは長文指示やプロンプトを絶対に書かない。他AIへの委託指示は「GitHubの `docs/DATA_COLLECTION_MASTER_GUIDE.md` （または raw URL）を読め」の1行のみ。何を集めるか・何を気をつけるか・過去の全ミス/フィードバック地雷録の統括正本は `docs/DATA_COLLECTION_MASTER_GUIDE.md`。
   ```
2. **プロジェクトのバケット設定（wrangler.jsonc）にR2バケットを接続**:
   - `RAW_BUCKET`: `foundation-raw`（原物保管庫）
   - `LAKE_BUCKET`: `foundation-lake`（保存票・Journal保管庫）
3. **台帳（D1またはJSON index）へのマッピング**:
   - 物理パスをいじらず、そのプロジェクトの主キーストア（例: Investraderの銘柄コード、Make-Moneyのentity ID）とR2オブジェクトを1行JOINする。

---

## Ⅵ. R2容量・料金の完全安全性（1万社収集でも無料枠内の数学的証明）

「データを大量に収集するとR2の料金が跳ね上がらないか」という懸念は、以下の客観的スペックにより数学的・構造的に完全に否定される：

1. **Cloudflare R2の完全無料枠（毎月リセット）**:
   - **ストレージ容量**: **10 GB / 月（完全無料）**
   - **書き込み（Class A）**: **1,000,000 回 / 月（完全無料）**
   - **読み取り（Class B）**: **10,000,000 回 / 月（完全無料）**
   - **データ転送（Egress）**: **完全無制限・永久無料（$0）**
2. **1社あたりの実データ量**:
   - 完全体JSON（約30KB）＋ 生原本魚拓・スクショ（約500KB〜1MB）＝ **最大1MB / 社**
3. **規模別コスト試算**:
   - **1,000社収集時**: 合計約0.8GB（無料枠の8%消費）➔ **月額0円（完全無料）**
   - **10,000社収集時**: 合計約8.0GB（無料枠10GB以内）➔ **月額0円（完全無料）**
   - **100,000社収集時（世界中を網羅）**: 合計約80GB ➔ **月額約240円（缶コーヒー2本分）**
4. **構造的防御機構**:
   - SHA-256 CASによる重複保存の自動遮断、物理移動ゼロ（Class A浪費ゼロ）、単一目録JOINによるクエリ局所化により、予期せぬ課金爆発は物理的に発生しない。

---

## Ⅶ. 外部AI過去重大やらかし事故リスト ＆ 再発防止規律（二度と繰り返すな）

外部AI（ChatGPT, Claude等）を用いてデータを大量収集・生成させる際は、必ず以下の過去重大事故リストをプロンプト冒頭に事前警告として叩き込め：

1. **為替レート逆数掛けバグ（数倍〜数十倍の計算狂い）**:
   - **事故**: TSMCでNTD/JPY=4.71477を誤って割り算（1/4.71477掛け）し、年商13兆円の企業を約6,000億円と出力した。
   - **防止策**: 外貨から日本円への換算は必ず「現地通貨額 × 為替レート＝日本円」である。逆数で割るな。
2. **タグラインのケタ・単位誤爆（100倍誤記）**:
   - **事故**: PayPalで年商4.96兆円であるにもかかわらず、タグラインで「年商497兆円」と表記した。
   - **防止策**: 本文・タグライン・P&Lの数値（億円・兆円）は必ず突き合わせ、ケタ・単位の矛盾をゼロにせよ。
3. **既存ID・名称・Tickerの衝突**:
   - **事故**: 37signalsのTickerを`BASECAMP`（別エンティティと衝突）、WeWorkで既存の検死レコードと同一名称で衝突した。
   - **防止策**: `id`, `name`, `ticker` はシステム全体で完全一意でなければならない。同一企業の別切り口（検死版等）を記録する場合は `WeWork (2019破綻前夜検死)` のように名称・IDを明確に区別せよ。
4. **未公開（UNAVAILABLE）時の未確認フラグ欠落**:
   - **事故**: Mailchimp等の非公開企業で売上未公開であるにもかかわらず、未確認フラグ（`isRevenueUnconfirmed: true`等）を付けずにバリデーションを落とした。
   - **防止策**: 財務未公開時は `financialStatus: 'UNAVAILABLE'` とし、`isRevenueUnconfirmed: true`, `isMarginUnconfirmed: true` を漏れなく付与せよ。
5. **スキーマ必須型の欠落**:
   - **事故**: `operations` 内の `weeklyHours`, `initialCapitalRequired`, `automationLevel` や、`evidenceCards` 内の `evidenceStatus`（`REPORTED` / `POST_MORTEM`）、`observations` と `observationsStream` の分離が漏れて型エラーになった。
   - **防止策**: `docs/GOLDEN_INGEST_SCHEMA.md` の完全体JSONスキーマを1文字たりとも漏らさず満たせ。

---

## Ⅷ. マルチチャット並列自走規律（排他予約 ＆ 隔離保存）

ユーザーが就寝中や複数チャット（別AI）を並列起動して事例を大量収集・精錬させる場合は、以下の排他ロックプロトコルを100%遵守せよ：

1. **調査直前の予約ロック（CLAIM_LOCK義務）**:
   - 調査を開始する直前に、必ず `data/CLAIMED_TARGETS.txt` を検索せよ。
   - 既に掲載されている企業は**「他のAIが現在調査中、または既に台帳登録済み」**であるため即座にスキップせよ。
   - 未掲載の新規企業を調べる場合は、**調べる前に必ず `data/CLAIMED_TARGETS.txt` の末尾に社名を1行だけ追記（Append）してから**調査に着手せよ（既存リストの上書き消去は厳禁）。
2. **直接台帳の編集禁止（`data/incoming/` への完全体JSON保存）**:
   - 中央台帳（`data/entities-index.json`）を外部チャットから直接編集・上書きすることを禁止する（Git衝突・マージ破綻の防止）。
   - 成果物は `data/incoming/batch_[チャット識別名]_[タイムスタンプ].json` に完全体JSON配列（`FinancialEntity[]`）として保存せよ。
   - 指揮官エージェントが定期的に `data/incoming/` をスキャンし、R2保管庫への原本格納および中央台帳への安全なアトミック統合を自律完遂する。

