...
test('structure visuals stay evidence-gated and do not surface fabricated margin claims', async ({ page }) => {
  ...
  await expect(valueChain).toBeInViewport();
  await expect(valueChain).toContainText(/提供経路|バリューチェーン比較は未確定/);
  await expect(valueChain).not.toContainText('40〜70%');
  await expect(valueChain).not.toContainText('中間マージン全量バイパス');
  await expect(valueChain).not.toContainText('利益独占');
  await expect(valueChain).not.toContainText('営業利益率へ全量転換');
  expect(errors).toEqual([]);
});
...
test('unconfirmed financials have an honest label and a working navigation target', async ({ page }) => {
  await page.goto('/?entity=ent_photoai');
  await expect(page.getByRole('heading', { name: 'Photo AI', exact: true })).toBeVisible();
  const jump = page.getByRole('button', { name: /^03\s*損益$/ });
  await expect(jump).toBeVisible();
  await jump.click();
  const section = page.locator('#section-cash-anatomy');
  await expect(section).toBeInViewport();
  await expect(section).toContainText(/財務データ.*未確認/);
