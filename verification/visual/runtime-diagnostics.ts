...
import { localVisualUrl, visualOrigin } from './origin-fixture';
...
export async function reportRuntimeFailure(page: Page, info: TestInfo): Promise<void> {
  ...
  for (const error of failures.get(page) || []) {
    const match = error.match(/(https:\/\/makemoney-app\.pages\.dev\/[^\s)]+\.js):(\d+):(\d+)/);
    if (!match || new URL(match[1]).origin !== visualOrigin) continue;
    const response = await page.request.get(localVisualUrl(match[1]));
